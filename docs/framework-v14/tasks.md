...
- **Acceptance Criteria**:
  - [x] All 4 E2E suites verified passing: sm-kms, jose-ja, sm-im, skeleton-template
  - [x] Factory-based TestMain initializes composeManager, sharedHTTPClient, sharedHTTPClientWithCA
  - [x] `docker compose down -v` after each run
  - [x] issuing-ca.pem generated by writeAdminCABundle; healthchecks pass admin mTLS
  - [x] TestE2E_PostgreSQLMTLS fixed: filter by client_dn instead of application_name
...
- **Acceptance Criteria**:
  ...
  - [x] v13 Item 7 explicitly closed (reference in commit message)
