...
1. ❌ Remove hardcoded port constants
2. ❌ Add server listener inspection to read assigned ports
3. ❌ Dynamically construct base URLs from assigned ports
4. ❌ Update all test HTTP client calls to use dynamic URLs
5. ❌ Ensure proper server startup ordering (start → read port → use port)
6. ❌ Handle port allocation failures gracefully
...
- Tests run sequentially → no port conflicts
...
- ✅ XSS test: Fixed (401 expectation corrected)
- ✅ Policy hot-reload: Fixed (value comparison instead of pointer)
- ✅ OAuth2 E2E: Fixed (sequential execution prevents port conflict)
...
- [ ] ~~Implement dynamic port allocation~~ → DEFERRED to backlog
...
2. ➡️ Apply sequential execution workaround
3. ➡️ Verify all tests pass
4. ➡️ Create P4.05-POSTMORTEM.md
5. ➡️ Mark P4.05 complete
6. ➡️ Continue to P4.04 (test coverage)
7. 📝 Add backlog issue for dynamic port allocation (after P4.08)
