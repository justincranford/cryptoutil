...
- [ ] **B)** Each pass checks ALL 8 quality attributes (Correctness, Completeness, Thoroughness, Reliability, Efficiency, Accuracy, NO Time Pressure, NO Premature Completion)
...
- [ ] **B)** Minimum 3, maximum 5 (continue if significant issues found in pass 3)
...
- [ ] **A)** Whenever Pass 3 finds ANY issue
...
- [ ] **C)** ALL work types (code, docs, config, tests, infrastructure, deployments)
...
- [ ] **A)** Generic continuous execution for ANY work type
- [ ] **B)** Primarily code tasks with some generic guidance
- [ ] **C)** Keep dual: generic principles + language-specific Quality Gates
...
- [ ] **A)** YES - keep them domain-specific (current design)
...
- [ ] **B)** Consolidate to Section 11.1, use @propagate to others, cross-reference from 1.3/2.5
...
- [ ] **C)** Consolidate to 4.4.7, make 9.1 a cross-reference
...
- [ ] **C)** Detailed tables only in 3.4, remove from Appendix B
...
- [ ] **A)** YES - add a new section about skills
...
- [ ] **B)** YES - add to Section 2.1 Agent Orchestration
...
- [ ] **A)** Section 12.7 (Documentation Propagation Strategy) - YES/NO
- [ ] **B)** Section 11.4 (Documentation Standards) - YES/NO
- [ ] **C)** Section B.6 (Instruction File Reference) - YES/NO
...
- [ ] **A)** YES - propagate relevant content like other instruction files
...
- [ ] **A)** YES - migrate suitable content from instructions/agents
...
- [ ] **C)** MAYBE - wait for more skill examples
...
- [ ] **C)** PARTIAL - skill wraps cicd lint-deployments
...
- [ ] **C)** PARTIAL - skill wraps cicd lint-docs
...
- [ ] **B)** REDUCE - target <4,000 lines through deduplication
...
- [ ] **A)** YES - automated validation (lint-docs) handles sync
...
- [ ] **A)** CONTINUE - helps navigation and avoids duplication
