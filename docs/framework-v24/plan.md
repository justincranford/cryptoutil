...
sm-kms APIs into sm-kms and sm-kms APIs into sm-kms, then deleting sm-kms, sm-kms, and the retired product.
...
- **Before**: 5 products (sm, pki, identity, skeleton), 10 PS-IDs
- **After**: 4 products (sm, pki, identity, skeleton), 8 PS-IDs

**Removed PS-IDs**: `sm-kms` (merged into `sm-kms`), `sm-kms` (merged into `sm-kms`)
**Removed Product**: `jose` (no remaining PS-IDs after removing sm-kms)

The consolidation preserves 100% of existing API surface. Every endpoint that existed in
sm-kms or sm-kms becomes a new endpoint group in sm-kms. No API functionality is deleted.
...
| sm-kms | sm | 8100 | 3001–3999 | ❌ Merged → sm-kms |
| sm-kms | sm | 8200 | 4001–4999 | ❌ Merged → sm-kms |
...
**sm-kms API endpoints unique vs sm-kms** (must be added to sm-kms):
...
**sm-kms domain objects** (must be ported as new sm-kms DB tables):
...
**sm-kms services** (must be ported to sm-kms):
...
**sm-kms API endpoints** (all new for sm-kms):
...
#### Phase 1: sm-kms → sm-kms API merge
...
#### Phase 2: sm-kms → sm-kms API merge
...
#### Phase 3: Delete sm-kms and sm-kms
...
internal/apps/sm/ja.go → DELETE entirely
```
...
```
api/cryptosuite-registry/registry.yaml     (remove sm-kms, sm-kms entries; remove retired product)
...
internal/apps-tools/cicd_lint/lint_ports/legacy_ports/*.go      (remove sm-kms/sm-kms legacy ports)
internal/apps-tools/cicd_lint/lint_fitness/registry/registry.go (remove sm-kms, sm-kms, retired product)
```
...
### Phase 1: sm-kms Domain → sm-kms (4d) [Status: ☐ TODO]

**Objective**: Port all sm-kms domain models, repositories, services, and API handlers into
sm-kms without deleting sm-kms yet. sm-kms and sm-kms both run; sm-kms is now redundant.
...
### Phase 2: sm-kms Domain → sm-kms (2d) [Status: ☐ TODO]
...
### Phase 3: Delete sm-kms, sm-kms, jose Product (1d) [Status: ☐ TODO]

**Objective**: Remove all sm-kms and sm-kms artifacts from the codebase. The retired product
...
5. Delete `cmd/sm-kms/`, `cmd/sm/`, `cmd/sm-kms/`
6. Delete `configs/sm-kms/`, `configs/sm-kms/`
7. Delete `deployments/sm-kms/`, `deployments/sm/`, `deployments/sm-kms/`
...
14. Delete `internal/apps/sm/` (product-level coordinator; retired product is gone)

- **Success**: `go build ./...` clean; no references to sm-kms or sm-kms anywhere; all remaining
...
7. Update `internal/apps-tools/cicd_lint/lint_ports/legacy_ports/`: remove sm-kms/sm-kms entries
...
5. Update `docs/DEV-SETUP.md` if any setup steps referenced sm-kms or sm-kms
...
- C: Create a single `server/apis/` directory (mimicking sm-kms/sm-kms structure)
- D:
- E:
...
| Fitness linter tests reference sm-kms/sm-kms by name | High | Medium | Phase 4 explicitly updates lint_ports and lint_fitness; run `lint-fitness` immediately after |
...
- [ ] No sm-kms or sm-kms references anywhere in non-documentation code
- [ ] sm-kms OpenAPI spec includes all sm-kms and sm-kms endpoints
