...
**Removed Product**: `jose` (no remaining PS-IDs after removing jose-ja)
...
jose-ja operates as a JWK Authority: JOSE-format key management (JWK containers keyed by `kid`,
...
sm-im is an Encrypted Instant Messenger: end-to-end encrypted messaging with JWE key wrapping
...
**New files (sm-kms receives jose-ja domain)**:
```
...
api/jose-ja/                       (7 files)
api/sm-im/                         (7 files)
internal/apps/jose-ja/             (75 files)
internal/apps/sm-im/               (60 files)
cmd/jose-ja/                       (1 file: main.go)
cmd/jose/                          (1 file: main.go)
cmd/sm-im/                         (1 file: main.go)
configs/jose-ja/                   (~5 files)
configs/sm-im/                     (~5 files)
deployments/jose-ja/               (~10 files)
deployments/jose/                  (~3 files)
deployments/sm-im/                 (~10 files)
...
```
deployments/sm/compose.yml                  (remove sm-im service)
deployments/cryptoutil/compose.yml          (remove jose service block)
cmd/sm/main.go                              (remove sm-im subcommand routing)
...
internal/apps-tools/cicd_lint/lint_ports/host_port_ranges/*.go  (remove jose-ja port range 8200s)
...
**Objective**: Port all jose-ja domain models, repositories, services, and API handlers into
sm-kms without deleting jose-ja yet. sm-kms and jose-ja both run; jose-ja is now redundant.

1. Add DB migrations 2003–2006 to sm-kms (elastic_jwks, material_jwks, audit_config, audit_log)
2. Port jose-ja domain models to `internal/apps/sm-kms/server/model/jwk_models.go`
3. Port jose-ja repositories to `internal/apps/sm-kms/server/repository/` (elastic_jwk_repo, material_jwk_repo, audit_repo)
4. Port jose-ja services to `internal/apps/sm-kms/server/jwkservice/` (elastic, rotation, JWKS, JWS, JWE, JWT, audit)
5. Add JWK handlers to `internal/apps/sm-kms/server/handler/` (JWK CRUD + rotate + active + JWKS endpoint)
6. Extend sm-kms OpenAPI spec (paths + components) with jose-ja API surface
...
10. Port jose-ja tests (adapted to sm-kms package structure)

- **Success**: `go build ./...` clean; all sm-kms tests pass; jose-ja tests still pass (both
...
**Objective**: Port all sm-im domain models, repository, and handler into sm-kms. sm-kms can
now send/receive encrypted messages.

1. Add DB migrations 2007–2008 to sm-kms (messages, messages_recipient_jwks)
2. Port sm-im domain models to `internal/apps/sm-kms/server/model/message_models.go`
3. Port sm-im repositories to `internal/apps/sm-kms/server/repository/` (message_repo, message_recipient_jwk_repo)
4. Port sm-im message handler to `internal/apps/sm-kms/server/handler/message_handler.go`
...
8. Port sm-im tests (adapted to sm-kms package structure)

- **Success**: `go build ./...` clean; all sm-kms tests pass (including messaging); sm-im tests
...
1. Delete `api/jose-ja/` entirely
2. Delete `api/sm-im/` entirely
3. Delete `internal/apps/jose-ja/` entirely
4. Delete `internal/apps/sm-im/` entirely
5. Delete `cmd/jose-ja/`, `cmd/jose/`, `cmd/sm-im/`
6. Delete `configs/jose-ja/`, `configs/sm-im/`
7. Delete `deployments/jose-ja/`, `deployments/jose/`, `deployments/sm-im/`
8. Delete `internal/shared/magic/magic_jose.go`, `magic_sm_im.go`
9. Update `deployments/sm/compose.yml` — remove sm-im service
10. Update `deployments/cryptoutil/compose.yml` — remove jose service block
11. Update `cmd/sm/main.go` — remove sm-im routing
...
1. Update `api/cryptosuite-registry/registry.yaml`:
   - Remove `jose-ja` from `product_services`
   - Remove `sm-im` from `product_services`
...
6. Update `internal/apps-tools/cicd_lint/lint_ports/host_port_ranges/`: remove jose-ja range (8200s)
...
**Rationale**: The migration range policy assigns ranges per PS-ID domain ownership. Once jose-ja
and sm-im are merged into sm-kms, their tables become sm-kms domain tables. Using 2003–2008
...
### Decision 2: jose-ja JWK Model vs sm-kms ElasticKey Model

**Options**:
- A: Add jose-ja's `elastic_jwks`/`material_jwks` tables alongside sm-kms's existing `elastic_keys`/`material_keys` ✓ **SELECTED**
- B: Unify into a single `elastic_keys` table by extending the schema with JWK-specific columns
- C: Replace sm-kms model with jose-ja's JWK model
- D: Wrap jose-ja JWK operations via sm-kms's existing elastic key APIs
- E:
...
- `elastic_jwks` / `material_jwks` (jose-ja): JOSE-format key management with kid, kty, alg, use
...
- C: Create a single `server/apis/` directory (mimicking sm-im/jose-ja structure)
- D:
- E:
...
### Decision 4: jose-ja Service Package Location in sm-kms
...
- C: Create `internal/apps/sm-kms/server/service/` (matches jose-ja's original package name)
- D:
- E:
...
| Deleted jose-ja import still referenced in shared packages | Medium | High | Run `grep -r jose-ja ./internal` after deletion before committing |
