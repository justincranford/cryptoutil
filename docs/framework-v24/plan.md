...
sm-kms APIs into sm-kms and sm-kms APIs into sm-kms, then deleting sm-kms, sm-kms, and the jose product.
...
| sm-kms | jose | 8200 | 4001–4999 | ❌ Merged → sm-kms |
...
```
api/cryptosuite-registry/registry.yaml     (remove sm-kms, sm-kms entries; remove jose product)
...
internal/apps-tools/cicd_lint/lint_fitness/registry/registry.go (remove sm-kms, sm-kms, jose product)
```
...
**Objective**: Remove all sm-kms and sm-kms artifacts from the codebase. The jose product
