...
CREATE INDEX IF NOT EXISTS idx_tenant_join_requests_user ON tenant_join_requests(user_id) WHERE user_id IS NOT NULL;

-- 1005_tenant_join_requests.down.sql
...
type TenantJoinRequestRepository interface {
    Create(ctx context.Context, request *domain.TenantJoinRequest) error
    GetByID(ctx context.Context, id googleUuid.UUID, tenantID googleUuid.UUID) (*domain.TenantJoinRequest, error)
    ListByTenant(ctx context.Context, tenantID googleUuid.UUID, status string, page, size int) ([]*domain.TenantJoinRequest, int, error)
    Update(ctx context.Context, request *domain.TenantJoinRequest) error
    DeleteOlderThan(ctx context.Context, cutoffDate time.Time) (int64, error)
}
```

**CRITICAL: Repository WHERE Clauses**:
- ✅ Filter by `tenant_id` (multi-tenancy isolation)
- ❌ NEVER filter by `realm_id` (realms are authn only, NOT data scope)

**Quality Gates**:
1. Build: `go build ./internal/apps/template/...`
2. Tests: `go test ./internal/apps/template/service/server/repository/... -cover`
3. Coverage: ≥98% (infrastructure code)
4. Mutation: ≥98% (gremlins score)
5. Evidence: Coverage report + mutation report + commit hash
...
type TenantRegistrationService interface {
    // User registration
    RegisterUserWithTenant(ctx context.Context, username, passwordHash string, tenantID *googleUuid.UUID) (*RegisterResult, error)

    // Service client registration
    RegisterClientWithTenant(ctx context.Context, clientID, clientSecretHash string, tenantID *googleUuid.UUID) (*RegisterResult, error)

    // Admin authorization
    AuthorizeJoinRequest(ctx context.Context, adminUserID, joinRequestID googleUuid.UUID, approve bool) error

    // List join requests
    ListJoinRequests(ctx context.Context, tenantID googleUuid.UUID, status string, page, size int) ([]*domain.TenantJoinRequest, int, error)
}
...
type RegisterResult struct {
    ...
    SessionToken string
    JoinRequestID *googleUuid.UUID  // Non-nil if joining existing tenant
}
```

**Logic**:
- If `tenantID == nil`: Create new tenant, create realm, create user/client, make admin, issue session
- If `tenantID != nil`: Create join request, return JoinRequestID (NO session until approved)

**CRITICAL: Password Handling**:
- ❌ NEVER use hardcoded passwords in tests
- ✅ ALWAYS use `cryptoutilMagic.TestPassword` or `googleUuid.NewV7().String()` for test passwords
- ✅ ALWAYS hash passwords before storage

**Quality Gates**:
1. Build: `go build ./internal/apps/template/...`
2. Tests: `go test ./internal/apps/template/service/server/businesslogic/... -cover`
3. Coverage: ≥95% (production code)
4. Mutation: ≥85% (gremlins score)
5. Security: NO hardcoded passwords in tests
6. Evidence: Coverage report + mutation report + commit hash
