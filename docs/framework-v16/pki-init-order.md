...
**Purpose**: Document why TLS wiring phases are ordered the way they are, and summarize what TLS work was completed before V15.
**Created**: 2026-04-16
...
PG leader→follower replication uses a server-only TLS channel initially. The full replication mTLS
(with follower client cert) is added AFTER app client mTLS is verified. This prevents replication
disruption from blocking app connectivity work.

For OTel/Grafana: the intra-infra channel (OTel→Grafana) is handled separately from the app→OTel
channel. OTel server TLS is established first, then app clients connect, then the OTel→Grafana
forwarding channel gets client cert.
...
All pki-init generator code and the following TLS wiring work was completed before V15:

### PostgreSQL mTLS (Completed)

**Scope**: PostgreSQL leader/follower server TLS → app client mTLS → replication mTLS → private PS-ID admin mTLS trust.

**Ordering applied**:
- Leader server TLS established and verified before replication server TLS — leader is write target; replication reconnects automatically
- Standalone server TLS verified (via `pg_stat_ssl`) before client mTLS added — isolates failures
- App client mTLS (Cat 14) enabled before replication client mTLS — replication disruption is more impactful; stable app path verified first
- Admin mTLS (Cat 6/7) sequenced after PG full stack — orthogonal concerns; kept separate

**Why Cat 9 infra cert generated with Cat 14**:
Cat 9 infra (OTel→Grafana client cert) is a prerequisite for the OTel/Grafana TLS wiring. It was generated alongside Cat 14 to allow OTel/Grafana work to begin without another pki-init patch cycle.

### OTel Collector + Grafana LGTM TLS (Completed)

**Scope**: OTel Collector server TLS (OTLP receiver) → app→OTel client mTLS → Grafana LGTM HTTPS UI + OTLP ingest TLS → OTel→Grafana client mTLS → public PS-ID app server TLS.

**Ordering applied**:
- OTel server TLS established and verified before app client mTLS — apps cannot present certs to a receiver that doesn't serve TLS
- Grafana HTTPS UI and OTLP ingest TLS combined into one phase — both use the same Cat 2 server cert; combining reduces container restarts
- OTel→Grafana client mTLS after Grafana HTTPS verified — staged: server first, then client
- Public PS-ID app server TLS (Cat 3) sequenced after telemetry pipeline — independent of telemetry; placed after to keep concerns cleanly separated

### pki-init Generator Code (All Completed)

**All 14 certificate categories** (Cat 1–14) are fully generated by `internal/apps/framework/tls/generator.go`.
No generator code changes are required for V15. V15 focuses on:

1. Comprehensive unit + integration tests for all 16 valid tier domain parameters
2. Wiring the generated certs into compose files, config files, and deployment templates
...
**Certificate categories used in V15**:
- **Cat 3** (PS-ID app server cert) → configure public HTTPS server TLS
- **Cat 4/5** (PS-ID HTTPS client CA/leaf) → configure client authentication trust
- **Cat 6/7** (private admin mTLS CA/leaf) → configure admin port mTLS
