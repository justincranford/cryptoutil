...
- **importas rule naming**: The golangci.yml importas rule for pi/pki-ca/server uses alias cryptoutilApiCaServer (not cryptoutilApiPkiCaServer) — the "pki" product name is dropped from the alias because the CA is the canonical PKI authority. Always verify importas aliases experimentally rather than guessing from package paths.- **api/ top-level layout: incomplete D21 migration, not cleanly dead code**: D21 originally planned to delete `api/client/`, `api/model/`, and root `generate.go` once per-service layouts were adopted, but that cleanup step was never done. Detailed breakdown by directory: (1) `api/client/` — legacy sm-kms generated HTTP client, used ONLY by `sm/kms/client/` (integration/e2e build tags) and `archived/e2e-legacy/`. NOT a shared library for other services — the previous claim that jose-ja uses it was INCORRECT. jose-ja uses `api/model/` (model types), not `api/client/`. (2) `api/model/` — legacy sm-kms generated models. WIDELY USED in production: sm-kms handler, businesslogic, repository, and `jose/ja/service/` and `shared/crypto/jose/` — do NOT delete until all ~40 import sites are migrated. (3) `api/common/` — DOES NOT EXIST. It never existed in git history. (4) `api/sm-kms/server/` — ACTIVE, used by sm-kms handler since Phase 10 migration — NOT migration debt. (5) `api/sm-kms/models/` — TRUE DEAD CODE: generated in Phase 10 but never imported by any Go file. Safe to delete once the models migration is intentionally pursued. (6) `api/sm-kms/client/` — TRUE DEAD CODE: same as above. (7) `api/jose-ja/client/` and `api/pki-ca/client/` — only referenced in `archived/e2e-legacy/`, effectively dead for production. The migration plan (D21) was: create per-service dirs first → then migrate all imports → then delete old top-level dirs. Only step 1 was done for sm-kms server; steps 2 and 3 remain outstanding.- **Parallel test races on package-level test seams**: Using a ar statFunc = os.Stat package-level seam causes data races when multiple parallel tests mutate it. For sub-linters that only need filesystem errors from a specific directory, use real tempdir tests with missing directories instead.
