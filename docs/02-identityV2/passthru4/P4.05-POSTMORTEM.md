...
1. **Fatal on startup errors** (immediate detection):
...
1. **Parallel shutdown with 10s timeout** (faster cleanup):
...
1. **Removed `t.Parallel()` from all test functions** (sequential execution):
...
1. **Added `sync` package import** (for `sync.WaitGroup`):
