...
**Priority**: 🔴 CRITICAL (blocks P4.04 coverage measurement)
...
- Actual unique test function failures: 3 ❌
...
**Status**: ⚠️ FALSE POSITIVE (test expectation incorrect, NOT security bug)
...
**Security Assessment**: ✅ NOT A SECURITY BUG
...
- Change `expectedStatus: http.StatusBadRequest` → `http.StatusUnauthorized`
...
- [ ] Change `expectedStatus` from 400 → 401 in XSS test case
...
**Status**: 🐛 TEST BUG (incorrect assertion, NOT production code bug)
...
**Production Impact**: ⚠️ MINOR ISSUE
...
**Status**: 🔥 PORT CONFLICT (test infrastructure bug, NOT application code bug)
...
5. Test attempts HTTP requests → HTTP 400 (server not running)
6. Cleanup phase tries to shutdown non-existent server
7. Database already closed → shutdown cleanup fails
...
**Production Impact**: ✅ NOT A PRODUCTION BUG
...
4. ➡️ **P4.05: Fix 3 test failures** (35-55 minutes)
...
2. ➡️ Fix #1: Update XSS test expectation (5 min)
3. ➡️ Fix #2: Investigate + fix policy hot-reload test (10-30 min)
4. ➡️ Fix #3: Implement dynamic port allocation in E2E (20 min)
5. ✅ Verify all tests pass: `go test ./internal/identity/... -count=1`
6. ✅ Create P4.05-POSTMORTEM.md
7. ➡️ Continue to P4.04 (test coverage)
...
3. **Test assertions should match behavior**: Pointer equality ≠ value equality
