...
- Multi-tenancy: Schema-level isolation ONLY (NOT row-level security)
...
**Objective**: Complete dual-server pattern for all services, add unified CLI, expand E2E coverage

### Phase 2.1: Admin Server Implementation

**Priority**: CRITICAL - Blocking unified CLI and production deployments

#### Task 2.1.1: JOSE Admin Server
...
1. Create `internal/jose/server/admin/` package
   - Admin server configuration (bind 127.0.0.1:9093)
...
3. Update Docker Compose
   - Health check: `wget --no-check-certificate -q -O /dev/null https://127.0.0.1:9093/admin/v1/livez`
...
1. Create `internal/ca/server/admin/` package
   - Admin server configuration (bind 127.0.0.1:9092)
   - Livez handler
...
**Timeline**: FUTURE
**Objective**: Validate template reusability, provide customer reference

**CRITICAL REQUIREMENT**: Learn-PS MUST be implemented FIRST before any production service migrations.

### Phase 7.1: Learn-PS Implementation

**Priority**: CRITICAL - Validates template before production migrations

#### Task 7.1.1: Pet Store Service
