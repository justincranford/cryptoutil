...
**Context**: MVP quality focus with service template extraction
...
**Goal**: Clean up AI slop from 001-cryptoutil, achieve MVP quality, extract reusable service template

**Approach**: 7-phase sequential execution with strict quality enforcement

**Timeline**: 160-240 hours work effort (210+ tasks)
...
- **CI/CD First**: Fix all 5 workflow failures before proceeding
...
**Objective**: ALL !integration packages execute in ≤12 seconds

**Rationale**: Fast feedback loops essential for development velocity. More aggressive target than 001-cryptoutil (was ≤15s).
...
## Phase 5: Refactor Hashes (Day 10-11, 16-24h)

**Objective**: Clean hash architecture with 4 types, 3 versions

**Rationale**: Current hash implementation scattered, no version management, no support for 4 types.
...
## Phase 6: Server Architecture Unification (Day 12-15, 48-72h)
...
## Phase 7: Learn-PS Demonstration Service (Day 16-18, 40-60h)

**Objective**: Create Pet Store service using extracted template as validation

**Rationale**: Template must be proven usable. Learn-PS serves as copy-paste-modify starting point for customers.
...
- ✅ Fast tests (≤12s per package)
- ✅ High coverage (95%+ production, 100% infra/util, NO EXCEPTIONS)
- ✅ Stable CI/CD (0 failures)
- ✅ High mutation kill (98%+ per package)
- ✅ Clean hash architecture (4 types, 3 versions)
