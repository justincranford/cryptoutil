...
1. **P1: JOSE** - JWK Authority for Cryptographic primitives (JWK, JWS, JWE, JWT)
2. **P2: Identity Product** - 5 microservices (AuthZ, IdP, Resource Server, Relying Party, Single Page Application)
3. **P3: Secrets Manager Product** - Key Management Service, with a sophisticated Barrier Service (Hierarchical key management: Unseal => Root => Intermediate => Content)
4. **P4: PKI Product** - Certificate authority with EST/OCSP/CRL support
...
**Purpose**: Internal admin tasks; shutdown, livez, readyz, metrics
**Bind**: `127.0.0.1:9090` (not externally accessible for ALL ENVIRONMENTS AND DEPLOYMENTS)
**Security**:
