...
func (s *ContentKeysService) EncryptContent(sqlTransaction *cryptoutilOrmRepository.OrmTransaction, clearContentBytes []byte) ([]byte, *googleUuid.UUID, error) {
	contentKeyKidUuid, clearContentKey, _, err := cryptoutilJose.GenerateEncryptionJWKFromPool(&cryptoutilJose.EncA256GCM, &cryptoutilJose.AlgDir, s.aes256KeyGenPool)
	if err != nil {
