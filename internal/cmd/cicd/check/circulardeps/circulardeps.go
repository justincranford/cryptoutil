...
// Check checks for circular dependencies in Go packages.
...
// CheckDependencies analyzes the JSON output from 'go list -json ./...' for circular dependencies.
// Returns an error if circular dependencies are found, nil if no circular dependencies exist.
func CheckDependencies(jsonOutput string) error {
