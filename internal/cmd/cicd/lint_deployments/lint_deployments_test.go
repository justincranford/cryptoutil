...
require.NoError(t, os.WriteFile(filepath.Join(baseDir, "compose.yml"), []byte("version: '3'"), 0o644))
require.NoError(t, os.WriteFile(filepath.Join(baseDir, "Dockerfile"), []byte("FROM alpine"), 0o644))
...
require.NoError(t, os.WriteFile(filepath.Join(baseDir, "secrets", secret), []byte("secret"), 0o644))
}
},
wantValid: true,
},
...
t.Helper()
require.NoError(t, os.WriteFile(filepath.Join(baseDir, "compose.yml"), []byte("version: '3'"), 0o644))
require.NoError(t, os.WriteFile(filepath.Join(baseDir, "init-db.sql"), []byte("CREATE TABLE..."), 0o644))
require.NoError(t, os.WriteFile(filepath.Join(baseDir, "README.md"), []byte("# Infrastructure"), 0o644))
},
wantValid: true,
},
}
...
require.NoError(t, os.WriteFile(filepath.Join(invalidDir, "compose.yml"), []byte("version: '3'"), 0o644))
