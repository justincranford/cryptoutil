...
`
	validPath := cryptoutilTestutil.WriteTempFile(t, tempDir, "ci-valid.yml", validContent)
	...
	build:
		...
		steps:
			- name: Do nothing
				run: echo "hello"
`
	invalidPath := cryptoutilTestutil.WriteTempFile(t, tempDir, "dast.yml", invalidContent)
...
`
	p := cryptoutilTestutil.WriteTempFile(t, tempDir, "ci-nolog.yml", content)
...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			path := cryptoutilTestutil.WriteTempFile(t, tempDir, tt.filename, tt.content)
...
func TestLoadActionExceptions_WithFile(t *testing.T) {
	...
	_ = cryptoutilTestutil.WriteTempFile(t, filepath.Dir(exceptionsFile), "workflows-outdated-action-exemptions.json", string(data))
...
`
	...
	workflowFile := cryptoutilTestutil.WriteTempFile(t, tempDir, "test.yml", content)
