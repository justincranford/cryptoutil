...
// Package rs provides the unified command interface for Identity Resource Server service.
package rs //nolint:wsl_v5
...
// Execute handles RS service commands matching other service patterns.
...
func printUsage() {
	fmt.Println("Usage: cryptoutil identity-rs <subcommand> [options]")
	fmt.Println("\nSubcommands:")
	fmt.Println("  start    - Start Identity Resource Server")
	fmt.Println("  stop     - Stop Identity Resource Server")
	fmt.Println("  status   - Check service status")
	fmt.Println("  health   - Check service health")
	fmt.Println("\nOptions:")
	fmt.Println("  --config, -c <path>  - Config file path (default: /app/run/identity-rs-docker.yml)")
}

// startService starts the Identity Resource Server.
func startService(parameters []string) {
	configFile := parseConfigFlag(parameters, "/app/run/identity-rs-docker.yml")

	fmt.Fprintf(os.Stderr, "Starting Identity Resource Server\n")
...
// stopService sends shutdown request to RS admin endpoint.
...
// statusService checks RS service readiness.
...
// healthService checks RS service health.
