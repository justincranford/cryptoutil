...
// Package idp provides the unified command interface for Identity Provider service.
package idp //nolint:wsl_v5
...
// Execute handles IDP service commands matching other service patterns.
...
func printUsage() {
	fmt.Println("Usage: cryptoutil identity-idp <subcommand> [options]")
	fmt.Println("\nSubcommands:")
	fmt.Println("  start    - Start Identity Provider service")
	fmt.Println("  stop     - Stop Identity Provider service")
	fmt.Println("  status   - Check service status")
	fmt.Println("  health   - Check service health")
	fmt.Println("\nOptions:")
	fmt.Println("  --config, -c <path>  - Config file path (default: /app/run/identity-idp-docker.yml)")
}

// startService starts the Identity Provider service.
func startService(parameters []string) {
	configFile := parseConfigFlag(parameters, "/app/run/identity-idp-docker.yml")

	fmt.Fprintf(os.Stderr, "Starting Identity Provider service\n")
	fmt.Fprintf(os.Stderr, "Using config file: %s\n", configFile)

	// Load IDP-specific configuration from YAML file.
	parseArgs := []string{"start", "--config", configFile}

	settings, err := cryptoutilIDPConfig.Parse(parseArgs, false)
	if err != nil {
...
// stopService sends shutdown request to IDP admin endpoint.
...
// statusService checks IDP service readiness.
...
// healthService checks IDP service health.
