...
type AuditLogRepository interface {
	...
	CreateWithSampling(ctx context.Context, entry *domain.AuditLogEntry, samplingRate float64) (bool, error)

	// GetByID retrieves an audit log entry by ID.
	GetByID(ctx context.Context, id googleUuid.UUID) (*domain.AuditLogEntry, error)

	// ListByTenantRealm retrieves audit log entries for a tenant/realm with pagination.
	ListByTenantRealm(ctx context.Context, tenantID, realmID googleUuid.UUID, offset, limit int) ([]domain.AuditLogEntry, error)

	// ListByOperation retrieves audit log entries for a specific operation with pagination.
	ListByOperation(ctx context.Context, tenantID googleUuid.UUID, operation string, offset, limit int) ([]domain.AuditLogEntry, error)

	// ListByResource retrieves audit log entries for a specific resource with pagination.
	ListByResource(ctx context.Context, resourceType, resourceID string, offset, limit int) ([]domain.AuditLogEntry, error)

	// ListByTimeRange retrieves audit log entries within a time range with pagination.
	ListByTimeRange(ctx context.Context, tenantID googleUuid.UUID, start, end time.Time, offset, limit int) ([]domain.AuditLogEntry, error)
