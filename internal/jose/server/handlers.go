...
type JWKGenerateRequest struct {
	Algorithm string `json:"algorithm"` // Algorithm: RSA4096, RSA3072, RSA2048, ECP521, ECP384, ECP256, OKPEd25519, Oct512, Oct384, Oct256.
...
func (s *Server) handleJWKGenerate(c *fiber.Ctx) error {
	...
	if !isValidAlgorithm(alg) {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{
			...
			"allowed": []string{"RSA4096", "RSA3072", "RSA2048", "ECP521", "ECP384", "ECP256", "OKPEd25519", "Oct512", "Oct384", "Oct256"},
		})
	}

	// Generate the JWK.
	kid, privateJWK, publicJWK, _, _, err := s.jwkGenService.GenerateJWK(&alg)
	if err != nil {
