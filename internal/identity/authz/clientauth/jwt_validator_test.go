...
import (
	...
	cryptoutilCommonKeyGen "cryptoutil/internal/common/crypto/keygen"
...
func TestPrivateKeyJWTValidator_ValidateJWT_Success(t *testing.T) {
	...
	rsaKeyPair, err := cryptoutilCommonKeyGen.GenerateRSAKeyPair(2048)
...
func TestPrivateKeyJWTValidator_ValidateJWT_InvalidSignature(t *testing.T) {
	...
	keyID := googleUuid.NewString()
	rsaKeyPair1, err := cryptoutilCommonKeyGen.GenerateRSAKeyPair(2048)
	require.NoError(t, err)
	...
	rsaKeyPair2, err := cryptoutilCommonKeyGen.GenerateRSAKeyPair(2048)
	require.NoError(t, err)
