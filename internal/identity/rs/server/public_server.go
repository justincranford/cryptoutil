...
// PublicServer represents the public RS HTTPS server.
...
// NewPublicServer creates a new public RS HTTPS server.
func NewPublicServer(
	...
	app := fiber.New(fiber.Config{
		AppName:       "RS Public Server",
		ServerHeader:  "RS-Public",
		StrictRouting: true,
		CaseSensitive: true,
	})
...
func (s *PublicServer) Start(ctx context.Context) error {
	...
	fmt.Printf("Starting RS Public Server on %s (actual port: %d)\n", addr, s.actualPort)
	...
	fmt.Printf("RS Public Server listening with TLS on %s\n", listener.Addr().String())
	...
	select {
	case <-ctx.Done():
		fmt.Println("Context cancelled, shutting down RS Public Server")
