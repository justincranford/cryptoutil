...
// NewPublicServer creates a new public IdP HTTPS server.
func NewPublicServer(
	...
	app := fiber.New(fiber.Config{
		AppName:       "IdP Public Server",
		ServerHeader:  "IdP-Public",
		StrictRouting: true,
		CaseSensitive: true,
	})
...
func (s *PublicServer) Start(ctx context.Context) error {
	...
	fmt.Printf("Starting IdP Public Server on %s (actual port: %d)\n", addr, s.actualPort)
	...
	fmt.Printf("IdP Public Server listening with TLS on %s\n", listener.Addr().String())
	...
	select {
	case <-ctx.Done():
		fmt.Println("Context cancelled, shutting down IdP Public Server")
