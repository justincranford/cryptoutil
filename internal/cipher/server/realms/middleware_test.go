...
import (
	...
	"cryptoutil/internal/cipher/server/util"
...
func initTestDB(t *testing.T) *gorm.DB {
	...
	// Migrate cipher-im domain tables.
...
func createTestPublicServer(t *testing.T, db *gorm.DB) (*server.PublicServer, string) {
	...
	srv, err := server.NewPublicServer(ctx, 0, userRepo, messageRepo, messageRecipientJWKRepo, jwkGenService, barrierService, jwtSecret, tlsCfg)
	require.NoError(t, err)
	...
	t.Cleanup(func() {
		...
		if err := srv.Shutdown(ctx); err != nil {
			t.Logf("createTestPublicServer cleanup: failed to shutdown server: %v", err)
		}
	})
