...
func TestCertificateChain(t *testing.T) {
	...
	tlsServerCert1 := testEndEntitySubject{SubjectName: "TLS Server 1", Duration: 397 * cryptoutilDateTime.Days1, DNSNames: []string{"localhost", "example.com"}, IPAddresses: []net.IP{net.ParseIP("127.0.0.1"), net.ParseIP("::1")}, KeyMaterial: testKeyMaterial{KeyPair: testKeyGenPool.Get()}}
	tlsClientCert1 := testEndEntitySubject{SubjectName: "TLS Client 1", Duration: 30 * cryptoutilDateTime.Days1, EmailAddresses: []string{"client@example.com"}, KeyMaterial: testKeyMaterial{KeyPair: testKeyGenPool.Get()}}
