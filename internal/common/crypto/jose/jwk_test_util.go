...
func GenerateAes256KeysForTest(t *testing.T, count int, enc *joseJwa.ContentEncryptionAlgorithm, alg *joseJwa.KeyEncryptionAlgorithm) []joseJwk.Key {
	...
	for range count {
		...
		go func() {
			defer wg.Done()
			_, jwk, _, err := GenerateAesJWK(enc, alg)
