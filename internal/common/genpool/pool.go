...
type KeyPoolConfig[T any] struct {
	ctx                 context.Context
...
type KeyGenPool[T any] struct {
	...
	permissionChannel     chan struct{}      // N generateWorker threads block wait before generating value, because value generation (e.g. RSA-4096) can be CPU & Memory expensive
	keyChannel            chan T             // N generateWorker threads publish generated Keys to this channel
...
func (c *KeyGenPool[T]) Name() string {
	return c.cfg.poolName
}

// NewPool supports finite or indefinite pools
func NewPool[T any](config *KeyPoolConfig[T]) (*KeyGenPool[T], error) {
...
func NewPoolConfig[T any](ctx context.Context, telemetryService *cryptoutilTelemetry.TelemetryService, poolName string, numWorkers uint32, poolSize uint32, maxLifetimeKeys uint64, maxLifetimeDuration time.Duration, generateFunction func() (T, error)) (*KeyPoolConfig[T], error) {
	config := &KeyPoolConfig[T]{
		ctx:                 ctx,
...
func validateConfig[T any](config *KeyPoolConfig[T]) error {
	if config == nil {
...
func (pool *KeyGenPool[T]) monitorShutdown() {
...
func (pool *KeyGenPool[T]) generateWorker(workerNum uint32) {
	startTime := time.Now()
	defer func() {
		if r := recover(); r != nil {
...
func (pool *KeyGenPool[T]) generateKeyAndReleasePermission(workerNum uint32, startTime time.Time) {
	defer func() {
		if r := recover(); r != nil {
...
func (pool *KeyGenPool[T]) Get() any {
	startTime := time.Now()
