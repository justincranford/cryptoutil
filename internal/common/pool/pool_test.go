...
var (
	...
	happyPathSize                = []uint32{1, 4, 25}
	happyPathMaxLifetimeValues   = []uint64{1, 100, MaxLifetimeValues}
	happyPathMaxLifetimeDuration = []time.Duration{MaxLifetimeDuration}
	happyPathGets                = []uint64{0, 1, 4, 100}
...
func TestPoolUUIDv7(t *testing.T) {
	for _, tc := range happyPathTestCases {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			poolConfig, err := NewValuePoolConfig(testCtx, testTelemetryService, tc.name, tc.workers, tc.size, tc.maxLifetimeValues, tc.maxLifetimeDuration, GenerateUUIDv7Function())
			require.NoError(t, err)
			require.NotNil(t, poolConfig)
			poolInstance, err := NewValuePool(poolConfig)
			require.NoError(t, err)
