...
func deriveJwksFromMChooseNCombinations(m [][]byte, chooseN int) ([]joseJwk.Key, error) {
	...
	for _, combination := range combinations {
		...
		_, jwk, _, err := cryptoutilJose.CreateAesJWKFromBytes(&kekKidUuid, &cryptoutilJose.AlgA256GCMKW, &cryptoutilJose.EncA256GCM, derivedKeyBytes) // use derived JWK for envelope encryption (i.e. A256GCM Key Wrap), not DIRECT encryption
		if err != nil {
