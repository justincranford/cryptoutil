...
// all cryptoutil services (sm-kms, sm-kms, identity-*, sm-kms, pki-ca, skeleton-template).
//
...
//	    return nil
//	}
type HandlerRegistration func(server cryptoutilAppsFrameworkServiceServer.IPublicServer) error

// PublicServerFactory creates a product-specific public server.
//
// Each service (sm-kms, sm-kms, identity-*, sm-kms, pki-ca, skeleton-template) provides its own factory
...
//
// Example sm-kms factory:
//
//	func NewPublicServerFromConfig(
//	    ctx context.Context,
//	    cfg *ApplicationConfig,
//	    template *cryptoutilTemplateServer.ServiceFramework,
...
type PublicServerFactory func(
	ctx context.Context,
	cfg *ApplicationConfig,
	template *cryptoutilAppsFrameworkServiceServer.ServiceFramework,
...
// - ServiceFrameworkServerSettings (bind addresses, ports, TLS, OTLP)
...
type ApplicationConfig struct {
	// ServiceFrameworkServerSettings contains common settings (bind addresses, TLS, OTLP, etc.).
...
//
// This is the primary entry point for all cryptoutil services (sm-kms, sm-kms, identity-*, sm-kms, pki-ca, skeleton-template).
//
...
// - cfg: Product-service configuration (ServiceFrameworkServerSettings, DB, handlers)
//
// Returns:
...
//
//	cfg := &ApplicationConfig{
//	    ServiceFrameworkServerSettings: cryptoutilConfig.NewTestConfig("127.0.0.1", 0, true),
//	    DB: testDB,
...
func StartApplicationListener(ctx context.Context, cfg *ApplicationConfig) (*ApplicationListener, error) {
	...
	} else if cfg.ServiceFrameworkServerSettings == nil {
		return nil, fmt.Errorf("config.ServiceFrameworkServerSettings cannot be nil")
	} else if cfg.DB == nil {
	...
	// Create ServiceFramework with shared infrastructure.
	// This initializes telemetry, JWK generation, and optionally barrier service.
	template, err := cryptoutilAppsFrameworkServiceServer.NewServiceFramework(ctx, cfg.ServiceFrameworkServerSettings, cfg.DB, cfg.DBType)
	if err != nil {
	...
	// Each product service (sm-kms, sm-kms, etc.) will need to provide:
...
func SendLivenessCheck(settings *cryptoutilAppsFrameworkServiceConfig.ServiceFrameworkServerSettings) ([]byte, error) {
...
func SendReadinessCheck(settings *cryptoutilAppsFrameworkServiceConfig.ServiceFrameworkServerSettings) ([]byte, error) {
...
func SendShutdownRequest(settings *cryptoutilAppsFrameworkServiceConfig.ServiceFrameworkServerSettings) error {
...
func (l *ApplicationListener) Config() *cryptoutilAppsFrameworkServiceConfig.ServiceFrameworkServerSettings {
	return l.config
}
...
func (l *ApplicationListener) SetApplicationForTesting(app *cryptoutilAppsFrameworkServiceServer.Application) {
	l.app = app
}
