...
func createTestTLSMaterial(t *testing.T) *cryptoutilAppsFrameworkServiceConfig.TLSMaterial {
	t.Helper()
	...
	tlsMaterial, err := cryptoutilAppsFrameworkServiceConfigTlsGenerator.GenerateTLSMaterial(tlsSettings)
	require.NoError(t, err)
...
func TestNewApplication_HappyPath(t *testing.T) {
	...
	adminServer := newMockAdminServer(cryptoutilSharedMagic.TestAdminPort, "https://localhost:9090")
...
func TestNewApplication_NilPublicServer(t *testing.T) {
	...
	app, err := cryptoutilAppsFrameworkServiceServer.NewApplication(ctx, nil, adminServer)
...
func TestApplication_Start_ContextCancelled(t *testing.T) {
	...
	startCtx, cancel := context.WithTimeout(ctx, cryptoutilSharedMagic.JoseJAMaxMaterials*time.Millisecond)
	defer cancel()
