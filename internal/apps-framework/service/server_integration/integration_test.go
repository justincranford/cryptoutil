...
func newIntegrationSQLiteDSN() string {
	return fmt.Sprintf("file:%s?mode=memory&cache=shared", googleUuid.New().String())
}
...
func TestIntegration_TenantRegistration_CreateTenant(t *testing.T) {
	...
	dsn := newIntegrationSQLiteDSN()

	gormDB, err := cryptoutilAppsFrameworkServiceServerRepository.InitSQLite(ctx, dsn, cryptoutilAppsFrameworkServiceServerRepository.MigrationsFS)
	...
	// Create repositories.
	tenantRepo := cryptoutilAppsFrameworkServiceServerRepository.NewTenantRepository(gormDB)
	userRepo := cryptoutilAppsFrameworkServiceServerRepository.NewUserRepository(gormDB)
	joinRequestRepo := cryptoutilAppsFrameworkServiceServerRepository.NewTenantJoinRequestRepository(gormDB)

	// Create registration service.
	registrationSvc := cryptoutilAppsFrameworkServiceServerBusinesslogic.NewTenantRegistrationService(
		gormDB,
		tenantRepo,
		userRepo,
		joinRequestRepo,
