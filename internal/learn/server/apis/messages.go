...
func (h *MessageHandler) HandleSendMessage() fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		senderID, ok := c.Locals(cryptoutilLearnServerUtil.ContextKeyUserID).(googleUuid.UUID)
		if !ok {
			//nolint:wrapcheck // Fiber framework error, wrapping not needed.
...
func (h *MessageHandler) HandleReceiveMessages() fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		recipientID, ok := c.Locals(cryptoutilLearnServerUtil.ContextKeyUserID).(googleUuid.UUID)
		if !ok {
			//nolint:wrapcheck // Fiber framework error, wrapping not needed.
			return c.Status(fiber.StatusUnauthorized).JSON(fiber.Map{
				"error": "authentication required",
...
func (h *MessageHandler) HandleDeleteMessage() fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		userID, ok := c.Locals(cryptoutilLearnServerUtil.ContextKeyUserID).(googleUuid.UUID)
		if !ok {
			//nolint:wrapcheck // Fiber framework error, wrapping not needed.
			return c.Status(fiber.StatusUnauthorized).JSON(fiber.Map{
				"error": "authentication required",
