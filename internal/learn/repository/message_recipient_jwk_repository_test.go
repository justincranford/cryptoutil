...
func TestMessageRecipientJWKRepository_Create(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantErr: true, // Empty JWK will fail barrier encryption validation
		},
		{
			...
			jwk: &domain.MessageRecipientJWK{
				...
				RecipientID: googleUuid.Must(googleUuid.NewV7()),
				MessageID:   googleUuid.Must(googleUuid.NewV7()),
				JWK:         `{"kty":"RSA","n":"` + string(make([]byte, 2048)) + `","e":"AQAB"}`,
			},
			wantErr: false,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			require.Equal(t, testJWK.JWK, retrieved.JWK, "JWK should decrypt to original value")
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantCount > 0 {
				...
				for _, jwk := range createdJWKs {
					expectedJWKs[jwk.JWK] = true
				}

				for _, retrieved := range retrieved {
					require.True(t, expectedJWKs[retrieved.JWK], "Retrieved JWK should match one of the created JWKs")
				}
			}
		})
	}
