...
func TestBuildInMemorySQLiteDB_WALPragmaFails(t *testing.T) {
	...
	closedOpen := func(_, _ string) (*sql.DB, error) {
		rawDB, openErr := sql.Open(cryptoutilSharedMagic.TestDatabaseSQLite, ":memory:")
		if openErr != nil {
			return nil, openErr
		}
	...
	require.Error(t, err)
	assert.Contains(t, err.Error(), "WAL pragma")
	assert.Nil(t, db)
	assert.Nil(t, sqlDB)
}
...
func TestBuildInMemorySQLiteDB_CloseOnWALError(t *testing.T) {
	...
	openThenClose := func(_, _ string) (*sql.DB, error) {
		rawDB, err := sql.Open(cryptoutilSharedMagic.TestDatabaseSQLite, ":memory:")
		if err != nil {
			return nil, err
		}
	...
	require.Error(t, err)
	assert.True(t, closed, "openThenClose should have been called")
}
