...
import (
	...
	. "cryptoutil/internal/apps/cicd/lint_fitness/cicd_coverage"
)
...
// createValidDir creates a temp directory with valid coverage files for all three required paths.
func createValidDir(t *testing.T) string {
	...
	for _, relPath := range []string{
                filepath.Join(cryptoutilSharedMagic.CICDExcludeDirGithubInstructions, "actions", "custom-cicd-lint", "action.yml"),
...
func TestCheckInDir_MissingActionFile(t *testing.T) {
	t.Parallel()
	...
	require.NoError(t, os.Remove(actionPath))
	...
	require.Error(t, err)
	require.Contains(t, err.Error(), "custom-cicd-lint/action.yml")
...
func TestCheckInDir_MissingLinterInAction(t *testing.T) {
	t.Parallel()
	...
	// Build action content that is missing the omit command.
	var lines []string
	...
	actionContent := strings.Join(lines, "\n")

	for _, entry := range []struct {
		relPath string
		content string
	}{
                {filepath.Join(cryptoutilSharedMagic.CICDExcludeDirGithubInstructions, "actions", "custom-cicd-lint", "action.yml"), actionContent},
...
func TestCheckInDir_MissingFormatterInPreCommit(t *testing.T) {
	...
	for _, entry := range []struct {
		...
		content string
                ...
                {filepath.Join(cryptoutilSharedMagic.CICDExcludeDirGithubInstructions, "actions", "custom-cicd-lint", "action.yml"), validContent},
