...
func TestCheckLeftoverCoverage_NoFiles(t *testing.T) {
	...
	err = os.WriteFile("main.go", []byte("package main"), cryptoutilSharedMagic.CacheFilePermissions)
	require.NoError(t, err)

	err = os.WriteFile("README.md", []byte("# Test"), cryptoutilSharedMagic.CacheFilePermissions)
	require.NoError(t, err)
...
func TestCheckLeftoverCoverage_WithCoverageFiles(t *testing.T) {
	...
	for _, cf := range coverageFiles {
		...
		err = os.WriteFile(cf, []byte("coverage data"), cryptoutilSharedMagic.CacheFilePermissions)
		require.NoError(t, err)
	}
