...
func TestIM_ServerSubcommand_Startup(t *testing.T) {
	...
	go func() {
		...
		require.Contains(t, output, "Starting cipher-im service")
		require.Contains(t, output, fmt.Sprintf("Public Server: https://127.0.0.1:%d", magic.DefaultPublicPortCipherIM))
		require.Contains(t, output, fmt.Sprintf("Admin Server:  https://127.0.0.1:%d", magic.DefaultPrivatePortCipherIM))
	...
	captureOutput(t, func() {
		exitCode := IM([]string{
			"health",
			"--url", fmt.Sprintf("https://127.0.0.1:%d/service/api/v1", magic.DefaultPublicPortCipherIM),
		})
...
func TestIM_ClientSubcommand_Help(t *testing.T) {
	...
	require.Contains(t, output, "Usage: cipher im client [options]")
...
func TestIM_InitSubcommand_Help(t *testing.T) {
	...
	require.Contains(t, output, "Usage: cipher im init [options]")
