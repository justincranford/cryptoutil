...
func createTriggerDB(t *testing.T, triggerSQL string) (*gorm.DB, *sql.DB) {
	...
	err = cryptoutilAppsSmImRepository.ApplySmIMMigrations(triggerSQLDB, cryptoutilAppsSmImRepository.DatabaseTypeSQLite)
	require.NoError(t, err)
...
func TestHandleReceiveMessages_MarkAsReadTriggerError(t *testing.T) {
	...
	jwkRepo := cryptoutilAppsSmImRepository.NewMessageRecipientJWKRepository(triggerDB, testBarrierService)
