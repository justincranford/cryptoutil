...
func TestIM_SubcommandHelpFlags(t *testing.T) {
	tests := []struct {
		...
		{
			subcommand: "client",
			helpTexts:  []string{"sm im client", "Run client operations"},
		},
		{
			subcommand: "init",
			helpTexts:  []string{"sm im init", "Initialize database schema"},
		},
		{
			subcommand: "health",
			helpTexts:  []string{"sm im health", "Check service health"},
		},
		{
			subcommand: "livez",
			helpTexts:  []string{"sm im livez", "Check service liveness"},
		},
		{
			subcommand: "readyz",
			helpTexts:  []string{"sm im readyz", "Check service readiness"},
		},
		{
			subcommand: "shutdown",
			helpTexts:  []string{"sm im shutdown", "Trigger graceful shutdown"},
		},
	}
...
func TestPrintIMVersion(t *testing.T) {
	...
	require.Contains(t, output, "sm-im service", "Version output should contain sm-im service")
}
