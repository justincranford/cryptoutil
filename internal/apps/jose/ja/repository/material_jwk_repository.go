...
type MaterialJWKRepository interface {
	...
	Create(ctx context.Context, materialJWK *cryptoutilAppsJoseJaModel.MaterialJWK) error

	// GetByMaterialKID retrieves a Material JWK by its material KID.
	GetByMaterialKID(ctx context.Context, materialKID string) (*cryptoutilAppsJoseJaModel.MaterialJWK, error)

	// GetByID retrieves a Material JWK by its UUID.
	GetByID(ctx context.Context, id googleUuid.UUID) (*cryptoutilAppsJoseJaModel.MaterialJWK, error)

	// GetActiveMaterial retrieves the active Material JWK for an Elastic JWK.
	GetActiveMaterial(ctx context.Context, elasticJWKID googleUuid.UUID) (*cryptoutilAppsJoseJaModel.MaterialJWK, error)

	// ListByElasticJWK retrieves all Material JWKs for an Elastic JWK.
	ListByElasticJWK(ctx context.Context, elasticJWKID googleUuid.UUID, offset, limit int) ([]*cryptoutilAppsJoseJaModel.MaterialJWK, int64, error)

	// RotateMaterial atomically retires the current active material and activates a new one.
	RotateMaterial(ctx context.Context, elasticJWKID googleUuid.UUID, newMaterial *cryptoutilAppsJoseJaModel.MaterialJWK) error
...
// Create stores a new Material JWK.
func (r *gormMaterialJWKRepository) Create(ctx context.Context, materialJWK *cryptoutilAppsJoseJaModel.MaterialJWK) error {
...
// GetByMaterialKID retrieves a Material JWK by its material KID.
func (r *gormMaterialJWKRepository) GetByMaterialKID(ctx context.Context, materialKID string) (*cryptoutilAppsJoseJaModel.MaterialJWK, error) {
	var materialJWK cryptoutilAppsJoseJaModel.MaterialJWK
...
// GetByID retrieves a Material JWK by its UUID.
func (r *gormMaterialJWKRepository) GetByID(ctx context.Context, id googleUuid.UUID) (*cryptoutilAppsJoseJaModel.MaterialJWK, error) {
	var materialJWK cryptoutilAppsJoseJaModel.MaterialJWK
	if err := r.db.WithContext(ctx).
		Where("id = ?", id.String()).
		First(&materialJWK).Error; err != nil {
...
// GetActiveMaterial retrieves the active Material JWK for an Elastic JWK.
func (r *gormMaterialJWKRepository) GetActiveMaterial(ctx context.Context, elasticJWKID googleUuid.UUID) (*cryptoutilAppsJoseJaModel.MaterialJWK, error) {
	var materialJWK cryptoutilAppsJoseJaModel.MaterialJWK
	if err := r.db.WithContext(ctx).
...
// ListByElasticJWK retrieves all Material JWKs for an Elastic JWK.
func (r *gormMaterialJWKRepository) ListByElasticJWK(ctx context.Context, elasticJWKID googleUuid.UUID, offset, limit int) ([]*cryptoutilAppsJoseJaModel.MaterialJWK, int64, error) {
	var (
		materialJWKs []*cryptoutilAppsJoseJaModel.MaterialJWK
		total        int64
	)
	...
	if err := r.db.WithContext(ctx).
		Model(&cryptoutilAppsJoseJaModel.MaterialJWK{}).
...
// RotateMaterial atomically retires the current active material and activates a new one.
func (r *gormMaterialJWKRepository) RotateMaterial(ctx context.Context, elasticJWKID googleUuid.UUID, newMaterial *cryptoutilAppsJoseJaModel.MaterialJWK) error {
	err := r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		// Retire current active material.
		now := time.Now().UTC()
		if err := tx.Model(&cryptoutilAppsJoseJaModel.MaterialJWK{}).
			Where("elastic_jwk_id = ? AND active = ?", elasticJWKID.String(), true).
...
func (r *gormMaterialJWKRepository) Delete(ctx context.Context, id googleUuid.UUID) error {
	if err := r.db.WithContext(ctx).
		Where("id = ?", id.String()).
		Delete(&cryptoutilAppsJoseJaModel.MaterialJWK{}).Error; err != nil {
