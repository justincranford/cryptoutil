...
func TestJWEService_Encrypt_UnsupportedAlgorithm(t *testing.T) {
	...
	elasticJWK, _, err := elasticSvc.CreateElasticJWK(ctx, tenantID, cryptoutilSharedMagic.JoseAlgEdDSA, cryptoutilAppsJoseJaDomain.KeyUseEnc, 10)
...
func TestJWEService_EncryptWithKID_UnsupportedAlgorithm(t *testing.T) {
	...
	jweSvc := NewJWEService(testElasticRepo, testMaterialRepo, testBarrierService)
	tenantID := googleUuid.New()
	elasticJWK, material, err := elasticSvc.CreateElasticJWK(ctx, tenantID, cryptoutilSharedMagic.JoseAlgEdDSA, cryptoutilAppsJoseJaDomain.KeyUseEnc, 10)
	require.NoError(t, err)
...
func TestJWEService_Encrypt_AlgorithmKeyMismatch(t *testing.T) {
	...
	require.NoError(t, err)
	err = testDB.Model(&cryptoutilAppsJoseJaDomain.ElasticJWK{}).Where("id = ?", elasticJWK.ID).Update("alg", "EC/P256").Error
	require.NoError(t, err)
	_, err = jweSvc.Encrypt(ctx, tenantID, elasticJWK.ID, []byte("test"))
	require.Error(t, err)
