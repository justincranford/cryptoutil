...
func TestMaterialRotationService_RotateMaterial(t *testing.T) {
	...
	elasticJWK, initialMaterial, err := elasticSvc.CreateElasticJWK(ctx, tenantID, cryptoutilMagic.JoseAlgRS256, joseJADomain.KeyUseSig, 5)
...
func TestMaterialRotationService_RotateMaterial_WrongTenant(t *testing.T) {
	...
	// Create elastic JWK.
	elasticJWK, _, err := elasticSvc.CreateElasticJWK(ctx, tenantID, cryptoutilMagic.JoseAlgRS256, joseJADomain.KeyUseSig, 5)
	require.NoError(t, err)
...
func TestMaterialRotationService_RotateMaterial_MaxReached(t *testing.T) {
	...
	elasticJWK, _, err := elasticSvc.CreateElasticJWK(ctx, tenantID, cryptoutilMagic.JoseAlgRS256, joseJADomain.KeyUseSig, 2)
	require.NoError(t, err)
...
func TestMaterialRotationService_RetireMaterial(t *testing.T) {
	...
	elasticJWK, initialMaterial, err := elasticSvc.CreateElasticJWK(ctx, tenantID, cryptoutilMagic.JoseAlgRS256, joseJADomain.KeyUseSig, 3)
	require.NoError(t, err)
...
func TestMaterialRotationService_RetireMaterial_WrongElasticJWK(t *testing.T) {
	...
	elasticJWK1, _, err := elasticSvc.CreateElasticJWK(ctx, tenantID, cryptoutilMagic.JoseAlgRS256, joseJADomain.KeyUseSig, 3)
	require.NoError(t, err)

	elasticJWK2, material2, err := elasticSvc.CreateElasticJWK(ctx, tenantID, cryptoutilMagic.JoseAlgRS256, joseJADomain.KeyUseSig, 3)
	require.NoError(t, err)
...
func TestMaterialRotationService_GetMaterialByKID_WrongElasticJWK(t *testing.T) {
	...
	// Create two elastic JWKs.
	elasticJWK1, _, err := elasticSvc.CreateElasticJWK(ctx, tenantID, cryptoutilMagic.JoseAlgRS256, joseJADomain.KeyUseSig, 5)
	require.NoError(t, err)

	_, material2, err := elasticSvc.CreateElasticJWK(ctx, tenantID, cryptoutilMagic.JoseAlgRS256, joseJADomain.KeyUseSig, 5)
	require.NoError(t, err)
...
func TestMaterialRotationService_RotateMaterial_AllAlgorithms(t *testing.T) {
	...
	tests := []struct {
		...
		{"RS384", cryptoutilMagic.JoseAlgRS384, joseJADomain.KeyUseSig},
		{"RS512", cryptoutilMagic.JoseAlgRS512, joseJADomain.KeyUseSig},
		{"PS256", cryptoutilMagic.JoseAlgPS256, joseJADomain.KeyUseSig},
		{"PS384", cryptoutilMagic.JoseAlgPS384, joseJADomain.KeyUseSig},
		{"PS512", cryptoutilMagic.JoseAlgPS512, joseJADomain.KeyUseSig},
		{"ES384", cryptoutilMagic.JoseAlgES384, joseJADomain.KeyUseSig},
		{"ES512", cryptoutilMagic.JoseAlgES512, joseJADomain.KeyUseSig},
		{"RSA3072Enc", cryptoutilMagic.JoseKeyTypeRSA3072, joseJADomain.KeyUseEnc},
		{"RSA4096Enc", cryptoutilMagic.JoseKeyTypeRSA4096, joseJADomain.KeyUseEnc},
		{"ECP384Enc", cryptoutilMagic.JoseKeyTypeECP384, joseJADomain.KeyUseEnc},
		{"ECP521Enc", cryptoutilMagic.JoseKeyTypeECP521, joseJADomain.KeyUseEnc},
		{"Oct192Enc", cryptoutilMagic.JoseKeyTypeOct192, joseJADomain.KeyUseEnc},
	}
