...
func TestSkeleton_TemplateService_RoutesCorrectly(t *testing.T) {
	...
	exitCode := Skeleton([]string{cryptoutilSharedMagic.SkeletonTemplateServiceName, cryptoutilSharedMagic.CLIHelpCommand}, nil, &stdout, &stderr)
	require.Equal(t, 0, exitCode)
...
func TestSkeleton_EntryPoint(t *testing.T) {
	...
	exitCode := Skeleton([]string{cryptoutilSharedMagic.CLIHelpFlag}, nil, &stdout, &stderr)
	require.Equal(t, 0, exitCode)
