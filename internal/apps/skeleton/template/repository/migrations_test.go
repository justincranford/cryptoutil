...
func TestApplySkeletonTemplateMigrations(t *testing.T) {
t.Parallel()
...
err = ApplySkeletonTemplateMigrations(sqlDB, DatabaseTypeSQLite)
require.NoError(t, err, "migrations should apply successfully")

// Verify template_items table was created.
var tableName string

err = sqlDB.QueryRowContext(ctx, "SELECT name FROM sqlite_master WHERE type='table' AND name='template_items'").Scan(&tableName)
require.NoError(t, err, "template_items table should exist")
require.Equal(t, "template_items", tableName)
}

func TestApplySkeletonTemplateMigrations_Error(t *testing.T) {
t.Parallel()
...
require.Contains(t, err.Error(), "failed to apply skeleton-template migrations")
}
