...
// MigrationsFS contains embedded skeleton-template specific migrations (11001+ only).
//
...
//   - 11001-11999: skeleton-template app-specific tables (template_items)
//
// CRITICAL: This embed ONLY contains skeleton-template specific migrations (11001+).
...
// This is used by tests to access all migrations (1001-1999 template + 11001+ skeleton-template) in sequence.
...
//
// Phase 2 - skeleton-template specific tables (11001+):
// - 11001_template_items: Minimal template demonstration table.
func ApplySkeletonTemplateMigrations(db *sql.DB, dbType DatabaseType) error {
	// Apply all migrations in sequence (1001-1999 template + 11001+ skeleton-template) using merged filesystem.
	...
	if err := runner.Apply(db, dbType); err != nil {
		return fmt.Errorf("failed to apply skeleton-template migrations (1001-1999 + 11001+): %w", err)
	}
