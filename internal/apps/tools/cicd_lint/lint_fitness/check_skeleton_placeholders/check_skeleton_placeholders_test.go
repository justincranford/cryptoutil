...
func TestFindViolations_CicdDirExcluded(t *testing.T) {
	...
	cicdDir := filepath.Join(tempDir, "internal", "apps", "tools", "cicd_lint")
	...
	violations, err := FindViolations(tempDir)
	require.NoError(t, err)
	require.Empty(t, violations, "internal/apps/tools/cicd_lint/ must be excluded")
}
...
func TestCheck_Integration(t *testing.T) {
	...
	logger := cryptoutilCmdCicdCommon.NewLogger("test-check-skeleton-placeholders")
