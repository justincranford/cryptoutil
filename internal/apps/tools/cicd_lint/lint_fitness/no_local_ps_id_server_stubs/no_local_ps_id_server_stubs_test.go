...
func TestFindViolations_NoViolations_NonTestFile(t *testing.T) {
	...
	dir := t.TempDir()
	psDir := filepath.Join(dir, "internal", "apps", "sm-kms", "server")
	require.NoError(t, os.MkdirAll(psDir, 0o755))

	// Production (non-test) file defining stub-like methods should NOT be flagged.
	require.NoError(t, os.WriteFile(filepath.Join(psDir, "server.go"), []byte(stubPublicServerGoSrc), 0o600))
...
`
	require.NoError(t, os.WriteFile(filepath.Join(psDir, "server_test.go"), []byte(content), 0o600))
...
func TestFindViolations_Violation_LocalAdminStub(t *testing.T) {
	...
	dir := t.TempDir()
	psDir := filepath.Join(dir, "internal", "apps", "jose-ja", "server")
	require.NoError(t, os.MkdirAll(psDir, 0o755))
...
`
	require.NoError(t, os.WriteFile(filepath.Join(fwDir, "application_test.go"), []byte(content), 0o600))
...
func (s *stubPublicServer) PublicBaseURL() string { return "" }
`
	require.NoError(t, os.WriteFile(filepath.Join(toolsDir, "foo_test.go"), []byte(content), 0o600))
...
func TestFindViolations_PartialMethodSet_NotFlagged(t *testing.T) {
	...
	dir := t.TempDir()
	psDir := filepath.Join(dir, "internal", "apps", "pki-ca", "server")
	require.NoError(t, os.MkdirAll(psDir, 0o755))
...
func TestCheckInDir_ReturnsError_WhenViolationsFound(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(filepath.Join(psDir, "server_test.go"), []byte(stubPublicServerGoSrc), 0o600))
