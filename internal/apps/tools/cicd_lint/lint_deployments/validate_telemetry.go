...
func validateOTLPEndpoints(entries []otlpConfigEntry, result *TelemetryValidationResult) {
	for _, entry := range entries {
		...
		if entry.Endpoint == "" {
			result.Errors = append(result.Errors,
				fmt.Sprintf("[ValidateTelemetry] '%s': otlp-endpoint is empty but otlp is enabled | See: ENG-HANDBOOK.md Section 9.4",
...
func validateEndpointFormat(entry otlpConfigEntry, result *TelemetryValidationResult) {
	...
	if err != nil {
		result.Errors = append(result.Errors,
			fmt.Sprintf("[ValidateTelemetry] '%s': invalid otlp-endpoint URL: %s | See: ENG-HANDBOOK.md Section 9.4",
	...
	if host == "" {
		result.Errors = append(result.Errors,
			fmt.Sprintf("[ValidateTelemetry] '%s': otlp-endpoint missing host: %s | See: ENG-HANDBOOK.md Section 9.4",
				filepath.Base(entry.FilePath), entry.Endpoint))
		result.Valid = false
	}
...
func validateOTLPServiceNames(entries []otlpConfigEntry, result *TelemetryValidationResult) {
	...
	for _, entry := range entries {
		...
		if firstFile, exists := seen[entry.Service]; exists {
			result.Warnings = append(result.Warnings,
				fmt.Sprintf("[ValidateTelemetry] Duplicate otlp-service '%s' in '%s' and '%s' | See: ENG-HANDBOOK.md Section 9.4",
