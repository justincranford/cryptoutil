	...
	for _, deployDir := range deployDirs {
		...
		if !configDirSet[configName] {
			...
			result.Errors = append(result.Errors,
				fmt.Sprintf("[ValidateMirror] deployment '%s' has no configs counterpart (expected configs/%s/) | See: ENG-HANDBOOK.md Section 12.4.11", deployDir, configName))
			result.Valid = false
		}
	}
	...
	for _, configDir := range configDirs {
		...
		if !found {
			...
			result.Warnings = append(result.Warnings,
				fmt.Sprintf("[ValidateMirror] config directory '%s' has no corresponding deployment (orphaned) | See: ENG-HANDBOOK.md Section 12.4.11", configDir))
		}
	}
