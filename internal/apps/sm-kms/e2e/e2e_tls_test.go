...
func TestE2E_TLSChainValidation(t *testing.T) {
	...
	tests := []struct {
		...
		publicURL string
		client    *http.Client
		wantOK    bool // true = expect HTTP 200, false = expect TLS error
	}{
		// Happy path: each instance with the correct pki-init CA.
		{
		...
		// Sad path: wrong CA (system roots only — pki-init CA is self-signed, not in system roots).
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			req, err := http.NewRequestWithContext(ctx, http.MethodGet, tt.publicURL+cryptoutilSharedMagic.KMSE2EHealthEndpoint, nil)
			...
			if tt.wantOK {
				...
				// Verify it's a TLS error (x509 cert verification failure), not a network error.
