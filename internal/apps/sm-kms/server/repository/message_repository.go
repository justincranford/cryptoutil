...
func (r *MessageRepository) Create(ctx context.Context, message *cryptoutilAppsSmKmsModel.Message) error {
	if err := cryptoutilAppsFrameworkServiceServerRepository.GetDB(ctx, r.db).WithContext(ctx).Create(message).Error; err != nil {
...
func (r *MessageRepository) FindByID(ctx context.Context, id googleUuid.UUID) (*cryptoutilAppsSmKmsModel.Message, error) {
	var message cryptoutilAppsSmKmsModel.Message
	if err := cryptoutilAppsFrameworkServiceServerRepository.GetDB(ctx, r.db).WithContext(ctx).First(&message, "id = ?", id).Error; err != nil {
...
func (r *MessageRepository) FindByRecipientID(ctx context.Context, recipientID googleUuid.UUID) ([]cryptoutilAppsSmKmsModel.Message, error) {
	var messages []cryptoutilAppsSmKmsModel.Message
...
func (r *MessageRepository) MarkAsRead(ctx context.Context, messageID googleUuid.UUID) error {
	if err := cryptoutilAppsFrameworkServiceServerRepository.GetDB(ctx, r.db).WithContext(ctx).
		Model(&cryptoutilAppsSmKmsModel.Message{}).
		Where("id = ?", messageID).
...
func (r *MessageRepository) Delete(ctx context.Context, id googleUuid.UUID) error {
	if err := cryptoutilAppsFrameworkServiceServerRepository.GetDB(ctx, r.db).WithContext(ctx).Delete(&cryptoutilAppsSmKmsModel.Message{}, "id = ?", id).Error; err != nil {
