...
func (s *jweServiceImpl) Encrypt(ctx context.Context, tenantID, elasticJWKID googleUuid.UUID, plaintext []byte) (string, error) {
	...
	if elasticJWK.Use != cryptoutilAppsSmKmsModel.KeyUseEnc {
...
func (s *jweServiceImpl) decryptWithMaterial(ctx context.Context, jweObject *jose.JSONWebEncryption, material *cryptoutilAppsSmKmsModel.MaterialJWK) ([]byte, error) {
	// Decode base64 encoded JWE string.
