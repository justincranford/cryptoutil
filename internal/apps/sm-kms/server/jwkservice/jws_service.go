...
func (s *jwsServiceImpl) Sign(ctx context.Context, tenantID, elasticJWKID googleUuid.UUID, payload []byte) (string, error) {
	...
	if elasticJWK.Use != cryptoutilAppsSmKmsModel.KeyUseSig {
...
func (s *jwsServiceImpl) signWithMaterial(ctx context.Context, elasticJWK *cryptoutilAppsSmKmsModel.ElasticJWK, material *cryptoutilAppsSmKmsModel.MaterialJWK, payload []byte) (string, error) {
...
func (s *jwsServiceImpl) verifyWithMaterial(ctx context.Context, jwsObject *jose.JSONWebSignature, material *cryptoutilAppsSmKmsModel.MaterialJWK) ([]byte, error) {
	// Decode base64 encoded JWE string.
