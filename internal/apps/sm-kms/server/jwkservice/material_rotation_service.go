...
type MaterialRotationService interface {
	...
	RotateMaterial(ctx context.Context, tenantID, elasticJWKID googleUuid.UUID) (*cryptoutilAppsSmKmsModel.MaterialJWK, error)
	...
	ListMaterials(ctx context.Context, tenantID, elasticJWKID googleUuid.UUID) ([]*cryptoutilAppsSmKmsModel.MaterialJWK, error)

	// GetActiveMaterial gets the currently active material for an elastic JWK.
	GetActiveMaterial(ctx context.Context, tenantID, elasticJWKID googleUuid.UUID) (*cryptoutilAppsSmKmsModel.MaterialJWK, error)

	// GetMaterialByKID gets a material by its KID (for decryption/verification).
	GetMaterialByKID(ctx context.Context, tenantID, elasticJWKID googleUuid.UUID, kid string) (*cryptoutilAppsSmKmsModel.MaterialJWK, error)
}
...
// RotateMaterial creates a new active material and marks previous as inactive.
func (s *materialRotationServiceImpl) RotateMaterial(ctx context.Context, tenantID, elasticJWKID googleUuid.UUID) (*cryptoutilAppsSmKmsModel.MaterialJWK, error) {
...
// ListMaterials lists all materials for an elastic JWK.
func (s *materialRotationServiceImpl) ListMaterials(ctx context.Context, tenantID, elasticJWKID googleUuid.UUID) ([]*cryptoutilAppsSmKmsModel.MaterialJWK, error) {
	// Verify tenant ownership.
	elasticJWK, err := s.elasticRepo.GetByID(ctx, elasticJWKID)
	if err != nil {
		return nil, fmt.Errorf("failed to get elastic JWK: %w", err)
...
// GetActiveMaterial gets the currently active material for an elastic JWK.
func (s *materialRotationServiceImpl) GetActiveMaterial(ctx context.Context, tenantID, elasticJWKID googleUuid.UUID) (*cryptoutilAppsSmKmsModel.MaterialJWK, error) {
	// Verify tenant ownership.
	elasticJWK, err := s.elasticRepo.GetByID(ctx, elasticJWKID)
	if err != nil {
		return nil, fmt.Errorf("failed to get elastic JWK: %w", err)
...
func (s *materialRotationServiceImpl) GetMaterialByKID(ctx context.Context, tenantID, elasticJWKID googleUuid.UUID, kid string) (*cryptoutilAppsSmKmsModel.MaterialJWK, error) {
	// Verify tenant ownership.
	elasticJWK, err := s.elasticRepo.GetByID(ctx, elasticJWKID)
	if err != nil {
		return nil, fmt.Errorf("failed to get elastic JWK: %w", err)
...
func (s *materialRotationServiceImpl) createMaterialJWK(ctx context.Context, elasticJWK *cryptoutilAppsSmKmsModel.ElasticJWK) (*cryptoutilAppsSmKmsModel.MaterialJWK, error) {
