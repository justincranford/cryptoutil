...
func (sm *SessionManager) issueJWSSession(ctx context.Context, isBrowser bool, principalID string, tenantID, realmID googleUuid.UUID) (string, error) {
	...
	if isBrowser {
		...
		var serviceJWK cryptoutilAppsFrameworkServiceServerRepository.ServiceSessionJWK
	...
	jwk, err := sm.jwkParseKeyFn(decryptedJWKBytes)
	if err != nil {
	...
	claimsBytes, err := sm.jsonMarshalFn(claims)
	if err != nil {
		return "", fmt.Errorf("failed to marshal JWT claims: %w", err)
	}

	// Sign JWT
	_, jwsBytes, err := sm.signBytesFn([]joseJwk.Key{jwk}, claimsBytes)
	if err != nil {
		return "", fmt.Errorf("failed to sign JWT: %w", err)
	}

	// Store session metadata in database (for revocation)
	tokenHash, err := sm.hashHighEntropyDeterministicFn(jti.String())
	if err != nil {
...
func (sm *SessionManager) validateJWSSession(ctx context.Context, isBrowser bool, token string) (any, error) {
	...
	// Parse JWK from JSON
	privateJWK, err := sm.jwkParseKeyFn(decryptedJWKBytes)
	if err != nil {
	...
	claimsBytes, err := sm.verifyBytesFn([]joseJwk.Key{publicJWK}, []byte(token))
	if err != nil {
