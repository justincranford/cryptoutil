...
func setupBarrierService(t *testing.T, db *gorm.DB, telemetrySvc *cryptoutilSharedTelemetry.TelemetryService, jwkGenSvc *cryptoutilSharedCryptoJose.JWKGenService) *cryptoutilAppsFrameworkServiceServerBarrier.Service {
	t.Helper()
	...
	barrierSvc, err := cryptoutilAppsFrameworkServiceServerBarrier.NewService(ctx, telemetrySvc, jwkGenSvc, barrierRepo, unsealService)
	require.NoError(t, err)
