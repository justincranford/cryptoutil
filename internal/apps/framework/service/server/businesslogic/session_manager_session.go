...
func (sm *SessionManager) ValidateBrowserSession(ctx context.Context, token string) (*cryptoutilAppsFrameworkServiceServerRepository.BrowserSession, error) {
	var (
		result any
		err    error
	)
	...
	browserSession, ok := result.(*cryptoutilAppsFrameworkServiceServerRepository.BrowserSession)
	if !ok {
...
func (sm *SessionManager) ValidateServiceSession(ctx context.Context, token string) (*cryptoutilAppsFrameworkServiceServerRepository.ServiceSession, error) {
	var (
		result any
		err    error
	)
	...
	serviceSession, ok := result.(*cryptoutilAppsFrameworkServiceServerRepository.ServiceSession)
	if !ok {
		return nil, fmt.Errorf("invalid session type returned")
	}
...
func (sm *SessionManager) CleanupExpiredSessions(ctx context.Context) error {
	...
	err := sm.db.WithContext(ctx).
		...
		Delete(&cryptoutilAppsFrameworkServiceServerRepository.BrowserSession{}).
		Error
	if err != nil {
	...
	err = sm.db.WithContext(ctx).
		Where("expiration < ? OR (last_activity IS NOT NULL AND last_activity < ?)", now, idleThreshold).
		Delete(&cryptoutilAppsFrameworkServiceServerRepository.ServiceSession{}).
		Error
	if err != nil {
...
func (sm *SessionManager) issueOPAQUESession(ctx context.Context, isBrowser bool, principalID string, tenantID, realmID googleUuid.UUID) (string, error) {
	...
	if isBrowser {
		browserSession := cryptoutilAppsFrameworkServiceServerRepository.BrowserSession{
			Session: session,
			UserID:  &principalID,
		}
		createErr = sm.db.WithContext(ctx).Create(&browserSession).Error
	} else {
		serviceSession := cryptoutilAppsFrameworkServiceServerRepository.ServiceSession{
			Session:  session,
