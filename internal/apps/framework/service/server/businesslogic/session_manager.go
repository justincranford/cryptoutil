...
func defaultBarrierEncrypt(ctx context.Context, barrier *cryptoutilAppsFrameworkServiceServerBarrier.Service, data []byte) ([]byte, error) {
	if barrier != nil {
...
func defaultBarrierDecrypt(ctx context.Context, barrier *cryptoutilAppsFrameworkServiceServerBarrier.Service, data []byte) ([]byte, error) {
	if barrier != nil {
...
func (sm *SessionManager) initializeSessionJWK(ctx context.Context, isBrowser bool, algorithm cryptoutilSharedMagic.SessionAlgorithmType) (googleUuid.UUID, error) {
	...
	var existingJWK cryptoutilAppsFrameworkServiceServerRepository.SessionJWK
	...
	newJWK := cryptoutilAppsFrameworkServiceServerRepository.SessionJWK{
		ID:           jwkID,
	...
	if isBrowser {
		browserJWK := cryptoutilAppsFrameworkServiceServerRepository.BrowserSessionJWK{SessionJWK: newJWK}
		createErr = sm.db.WithContext(ctx).Create(&browserJWK).Error
	} else {
		serviceJWK := cryptoutilAppsFrameworkServiceServerRepository.ServiceSessionJWK{SessionJWK: newJWK}
	...
	var canonicalJWK cryptoutilAppsFrameworkServiceServerRepository.SessionJWK
