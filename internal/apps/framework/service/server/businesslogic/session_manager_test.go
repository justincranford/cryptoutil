...
func TestSessionManager_IssueServiceSession_OPAQUE_Success(t *testing.T) {
	...
	// Verify session stored in database
	var session cryptoutilAppsFrameworkServiceServerRepository.ServiceSession
...
func TestSessionManager_CleanupExpiredSessions_ExpiredByTime(t *testing.T) {
	...
	sm.db.Model(&cryptoutilAppsFrameworkServiceServerRepository.BrowserSession{}).Where("user_id = ?", userID).Count(&count)
