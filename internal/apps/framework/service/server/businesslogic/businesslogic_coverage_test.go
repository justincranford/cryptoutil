...
type errorUserRepo struct {
	cryptoutilAppsFrameworkServiceServerRepository.UserRepository
	createErr error
}

func (r *errorUserRepo) Create(_ context.Context, _ *cryptoutilAppsFrameworkServiceServerRepository.User) error {
	if r.createErr != nil {
		return r.createErr
	}
...
type errorJoinRequestRepo struct {
	cryptoutilAppsFrameworkServiceServerRepository.TenantJoinRequestRepository
	createErr error
	updateErr error
}

func (r *errorJoinRequestRepo) Create(_ context.Context, _ *cryptoutilAppsFrameworkServiceServerDomain.TenantJoinRequest) error {
	if r.createErr != nil {
		return r.createErr
	}
...
func (r *errorJoinRequestRepo) Update(_ context.Context, _ *cryptoutilAppsFrameworkServiceServerDomain.TenantJoinRequest) error {
	if r.updateErr != nil {
		return r.updateErr
	}
...
func TestRegisterUserWithTenant_UserCreate_Error(t *testing.T) {
	...
	realUserRepo := cryptoutilAppsFrameworkServiceServerRepository.NewUserRepository(testDB)
...
func TestRegisterClientWithTenant_JoinRequest_Create_Error(t *testing.T) {
	...
	mockJoinRequestRepo := &errorJoinRequestRepo{
		TenantJoinRequestRepository: cryptoutilAppsFrameworkServiceServerRepository.NewTenantJoinRequestRepository(testDB),
...
func TestAuthorizeJoinRequest_Update_Error(t *testing.T) {
	...
	realJoinRequestRepo := cryptoutilAppsFrameworkServiceServerRepository.NewTenantJoinRequestRepository(testDB)
	mockJoinRequestRepo := &errorJoinRequestRepo{
	...
	joinRequest := &cryptoutilAppsFrameworkServiceServerDomain.TenantJoinRequest{
		ID:          requestID,
		ClientID:    &clientID,
		TenantID:    tenantID,
...
func signCustomJWSClaims(t *testing.T, sm *SessionManager, claimsJSON []byte) string {
	...
	var browserJWK cryptoutilAppsFrameworkServiceServerRepository.BrowserSessionJWK
