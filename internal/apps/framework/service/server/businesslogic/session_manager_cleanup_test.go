...
func TestSessionManager_CleanupServiceSessions(t *testing.T) {
	...
	expiredTime := time.Now().UTC().Add(-1 * time.Hour)
	updateErr := sm.db.Model(&cryptoutilAppsFrameworkServiceServerRepository.ServiceSession{}).
	...
	var count int64
	sm.db.Model(&cryptoutilAppsFrameworkServiceServerRepository.ServiceSession{}).
		Where("client_id = ?", clientID).
		Count(&count)
