...
type VerificationService interface {
	...
	ListPendingUsers(ctx context.Context, tenantID googleUuid.UUID) ([]*cryptoutilAppsFrameworkServiceServerRepository.UnverifiedUser, error)

	// ListPendingClients lists all unverified clients for a tenant.
	ListPendingClients(ctx context.Context, tenantID googleUuid.UUID) ([]*cryptoutilAppsFrameworkServiceServerRepository.UnverifiedClient, error)

	// ApproveUser approves a pending user registration and assigns roles.
	ApproveUser(ctx context.Context, tenantID, unverifiedUserID googleUuid.UUID, roleIDs []googleUuid.UUID) (*cryptoutilAppsFrameworkServiceServerRepository.User, error)

	// ApproveClient approves a pending client registration and assigns roles.
	ApproveClient(ctx context.Context, tenantID, unverifiedClientID googleUuid.UUID, roleIDs []googleUuid.UUID) (*cryptoutilAppsFrameworkServiceServerRepository.Client, error)
...
// ListPendingUsers lists all unverified users for a tenant.
func (s *VerificationServiceImpl) ListPendingUsers(ctx context.Context, tenantID googleUuid.UUID) ([]*cryptoutilAppsFrameworkServiceServerRepository.UnverifiedUser, error) {
...
// ListPendingClients lists all unverified clients for a tenant.
func (s *VerificationServiceImpl) ListPendingClients(ctx context.Context, tenantID googleUuid.UUID) ([]*cryptoutilAppsFrameworkServiceServerRepository.UnverifiedClient, error) {
...
// ApproveUser approves a pending user registration and assigns roles.
func (s *VerificationServiceImpl) ApproveUser(ctx context.Context, tenantID, unverifiedUserID googleUuid.UUID, roleIDs []googleUuid.UUID) (*cryptoutilAppsFrameworkServiceServerRepository.User, error) {
	...
	// Assign roles.
	for _, roleID := range roleIDs {
		userRole := &cryptoutilAppsFrameworkServiceServerRepository.UserRole{
			TenantID: tenantID,
			UserID:   user.ID,
			RoleID:   roleID,
		}
...
// ApproveClient approves a pending client registration and assigns roles.
func (s *VerificationServiceImpl) ApproveClient(ctx context.Context, tenantID, unverifiedClientID googleUuid.UUID, roleIDs []googleUuid.UUID) (*cryptoutilAppsFrameworkServiceServerRepository.Client, error) {
	...
	client := &cryptoutilAppsFrameworkServiceServerRepository.Client{
		ID:               googleUuid.New(),
	...
	// Assign roles.
	for _, roleID := range roleIDs {
		clientRole := &cryptoutilAppsFrameworkServiceServerRepository.ClientRole{
			TenantID: tenantID,
			ClientID: client.ID,
			RoleID:   roleID,
		}
