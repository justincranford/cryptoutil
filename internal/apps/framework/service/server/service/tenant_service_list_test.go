...
func TestTenantService_ListTenants(t *testing.T) {
	...
	activeTenant := &cryptoutilAppsFrameworkServiceServerRepository.Tenant{
		ID:     googleUuid.New(),
		Name:   "active-tenant",
		Active: 1,
	}
	inactiveTenant := &cryptoutilAppsFrameworkServiceServerRepository.Tenant{
		ID:     googleUuid.New(),
	...
	tests := []struct {
	...
	}{
		{
			...
			setupMocks: func(tenantRepo *mockTenantRepository) {
				tenantRepo.listFn = func(_ context.Context, _ bool) ([]*cryptoutilAppsFrameworkServiceServerRepository.Tenant, error) {
					return []*cryptoutilAppsFrameworkServiceServerRepository.Tenant{activeTenant, inactiveTenant}, nil
				}
			},
			wantCount: 2,
			wantErr:   false,
		},
		{
			...
			setupMocks: func(tenantRepo *mockTenantRepository) {
				tenantRepo.listFn = func(_ context.Context, _ bool) ([]*cryptoutilAppsFrameworkServiceServerRepository.Tenant, error) {
					return []*cryptoutilAppsFrameworkServiceServerRepository.Tenant{activeTenant}, nil
				}
			},
			wantCount: 1,
			wantErr:   false,
...
func TestTenantService_UpdateTenant_UpdateError(t *testing.T) {
	...
	existingTenant := &cryptoutilAppsFrameworkServiceServerRepository.Tenant{
		ID:          tenantID,
		Name:        "old-name",
	...
	tenantRepo := &mockTenantRepository{
		getByIDFn: func(_ context.Context, _ googleUuid.UUID) (*cryptoutilAppsFrameworkServiceServerRepository.Tenant, error) {
			return existingTenant, nil
		},
		updateFn: func(_ context.Context, _ *cryptoutilAppsFrameworkServiceServerRepository.Tenant) error {
...
func TestTenantService_GetTenantByName(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name: "happy path",
			setupMocks: func(tenantRepo *mockTenantRepository) {
				tenantRepo.getByNameFn = func(_ context.Context, _ string) (*cryptoutilAppsFrameworkServiceServerRepository.Tenant, error) {
					return tenant, nil
				}
			},
			wantErr: false,
