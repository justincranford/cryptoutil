...
func TestVerificationService_ApproveClient_Success(t *testing.T) {
	...
	db.Model(&cryptoutilAppsFrameworkServiceServerRepository.UnverifiedClient{}).Where("id = ?", unverifiedClient.ID).Count(&count)
...
func TestVerificationService_CleanupExpiredRegistrations(t *testing.T) {
	...
	expiredUser := &cryptoutilAppsFrameworkServiceServerRepository.UnverifiedUser{
		ID:           googleUuid.New(),
		TenantID:     tenant.ID,
	...
	validUser := &cryptoutilAppsFrameworkServiceServerRepository.UnverifiedUser{
		ID:           googleUuid.New(),
		TenantID:     tenant.ID,
	...
	expiredClient := &cryptoutilAppsFrameworkServiceServerRepository.UnverifiedClient{
		ID:               googleUuid.New(),
		TenantID:         tenant.ID,
	...
	db.Model(&cryptoutilAppsFrameworkServiceServerRepository.UnverifiedUser{}).Where("id = ?", expiredUser.ID).Count(&expiredUserCount)
	require.Equal(t, int64(0), expiredUserCount)

	var expiredClientCount int64

	db.Model(&cryptoutilAppsFrameworkServiceServerRepository.UnverifiedClient{}).Where("id = ?", expiredClient.ID).Count(&expiredClientCount)
	require.Equal(t, int64(0), expiredClientCount)

	// Verify valid user still exists.
	var validUserCount int64

	db.Model(&cryptoutilAppsFrameworkServiceServerRepository.UnverifiedUser{}).Where("id = ?", validUser.ID).Count(&validUserCount)
