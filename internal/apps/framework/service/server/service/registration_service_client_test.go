	...
	tests := []struct {
	...
	}{
		{
			...
			setupMocks: func(unverifiedUserRepo *mockUnverifiedUserRepository) {
				unverifiedUserRepo.createFn = func(ctx context.Context, user *cryptoutilAppsFrameworkServiceServerRepository.UnverifiedUser) error {
					return nil
				}
			},
...
func TestRegistrationService_RegisterClient_NewTenant(t *testing.T) {
	...
	tenantSvc := &mockTenantService{
		createTenantFn: func(ctx context.Context, name, description string) (*cryptoutilAppsFrameworkServiceServerRepository.Tenant, error) {
			return &cryptoutilAppsFrameworkServiceServerRepository.Tenant{ID: tenantID, Name: name, Active: 1}, nil
		},
	}
