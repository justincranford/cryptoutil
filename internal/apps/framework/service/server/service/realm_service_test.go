...
func setupRealmService(t *testing.T) (RealmService, *gorm.DB) {
	...
	realmRepo := cryptoutilAppsFrameworkServiceServerRepository.NewTenantRealmRepository(db)
...
func createRealmTestTenant(t *testing.T, db *gorm.DB, tenantName string) *cryptoutilAppsFrameworkServiceServerRepository.Tenant {
	t.Helper()
