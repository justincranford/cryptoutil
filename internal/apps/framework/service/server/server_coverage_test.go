...
func TestNewServiceFramework_OptionError(t *testing.T) {
	...
	failingOption := func(_ *ServiceFramework) error {
		return fmt.Errorf("intentional option failure")
	}

	_, err = NewServiceFramework(ctx, config, db, cryptoutilAppsFrameworkServiceServerRepository.DatabaseTypeSQLite, failingOption)
	require.Error(t, err)
...
func TestServiceFramework_ShutdownNilComponents(t *testing.T) {
	...
	// Create ServiceFramework with all nil components.
	st := &ServiceFramework{
		telemetry: nil,
		jwkGen:    nil,
	}
