...
var (
	...
	testService          *cryptoutilAppsFrameworkServiceServerBarrier.Service
...
func TestMain(m *testing.M) {
	...
	testTelemetryService, err = cryptoutilSharedTelemetry.NewTelemetryService(ctx, cryptoutilAppsFrameworkServiceConfig.NewTestConfig(cryptoutilSharedMagic.IPv4Loopback, 0, true).ToTelemetrySettings())
	if err != nil {
	...
	testService, err = cryptoutilAppsFrameworkServiceServerBarrier.NewService(
		ctx,
...
func TestService_Shutdown(t *testing.T) {
	...
	barrierRepo, err := cryptoutilAppsFrameworkServiceServerBarrier.NewGormRepository(shutdownDB)
	require.NoError(t, err)
	t.Cleanup(func() { barrierRepo.Shutdown() })

	service, err := cryptoutilAppsFrameworkServiceServerBarrier.NewService(
		ctx,
		telemetrySvc,
		jwkGenSvc,
		barrierRepo,
