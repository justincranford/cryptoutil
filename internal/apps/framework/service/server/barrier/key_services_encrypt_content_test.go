...
func TestContentKeysService_EncryptContent_ErrorPaths(t *testing.T) {
	...
	t.Run("intermediate_key_encryption_fails", func(t *testing.T) {
		...
		contentKeysSvc, err := cryptoutilAppsFrameworkServiceServerBarrier.NewContentKeysService(telemetrySvc, jwkGenSvc, repo, intermediateKeysSvc)
		require.NoError(t, err)
		...
		err = repo.WithTransaction(ctx, func(tx cryptoutilAppsFrameworkServiceServerBarrier.Transaction) error {
	...
	t.Run("add_content_key_db_failure", func(t *testing.T) {
		...
		err = repo.WithTransaction(ctx, func(tx cryptoutilAppsFrameworkServiceServerBarrier.Transaction) error {
			return tx.AddContentKey(&cryptoutilAppsFrameworkServiceServerBarrier.ContentKey{
...
func TestContentKeysService_DecryptContent_ErrorPaths(t *testing.T) {
	...
	t.Run("decrypt_content_key_failure", func(t *testing.T) {
		...
		rootKeysSvc1, err := cryptoutilAppsFrameworkServiceServerBarrier.NewRootKeysService(telemetrySvc, jwkGenSvc, repo, unsealSvc1)
		require.NoError(t, err)
		t.Cleanup(func() { rootKeysSvc1.Shutdown() })

		intermediateKeysSvc1, err := cryptoutilAppsFrameworkServiceServerBarrier.NewIntermediateKeysService(telemetrySvc, jwkGenSvc, repo, rootKeysSvc1)
		require.NoError(t, err)
		t.Cleanup(func() { intermediateKeysSvc1.Shutdown() })

		contentKeysSvc1, err := cryptoutilAppsFrameworkServiceServerBarrier.NewContentKeysService(telemetrySvc, jwkGenSvc, repo, intermediateKeysSvc1)
		require.NoError(t, err)
		...
		rootKeysSvc2, err := cryptoutilAppsFrameworkServiceServerBarrier.NewRootKeysService(telemetrySvc, jwkGenSvc, repo, unsealSvc2)
		require.NoError(t, err)
		t.Cleanup(func() { rootKeysSvc2.Shutdown() })

		intermediateKeysSvc2, err := cryptoutilAppsFrameworkServiceServerBarrier.NewIntermediateKeysService(telemetrySvc, jwkGenSvc, repo, rootKeysSvc2)
		require.NoError(t, err)
		t.Cleanup(func() { intermediateKeysSvc2.Shutdown() })

		contentKeysSvc2, err := cryptoutilAppsFrameworkServiceServerBarrier.NewContentKeysService(telemetrySvc, jwkGenSvc, repo, intermediateKeysSvc2)
		require.NoError(t, err)
