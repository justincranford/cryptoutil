...
func TestGormRepository_RootKey_Lifecycle(t *testing.T) {
	...
	err = barrierRepo.WithTransaction(ctx, func(tx cryptoutilAppsFrameworkServiceServerBarrier.Transaction) error {
		latest, err := tx.GetRootKeyLatest()
		require.ErrorIs(t, err, cryptoutilAppsFrameworkServiceServerBarrier.ErrNoRootKeyFound, "Should get ErrNoRootKeyFound when no root keys exist")
	...
	key1 := &cryptoutilAppsFrameworkServiceServerBarrier.RootKey{
		UUID:      key1UUID,
	...
	key2 := &cryptoutilAppsFrameworkServiceServerBarrier.RootKey{
		UUID:      key2UUID,
...
func TestGormRepository_IntermediateKey_Lifecycle(t *testing.T) {
	...
	rootKey := &cryptoutilAppsFrameworkServiceServerBarrier.RootKey{
		UUID:      rootKeyUUID,
		Encrypted: "encrypted_root_key_1",
		KEKUUID:   googleUuid.UUID{},
	}
	...
	err = barrierRepo.WithTransaction(ctx, func(tx cryptoutilAppsFrameworkServiceServerBarrier.Transaction) error {
		...
		require.ErrorIs(t, err, cryptoutilAppsFrameworkServiceServerBarrier.ErrNoIntermediateKeyFound, "Should get ErrNoIntermediateKeyFound when no intermediate keys exist")
		require.Nil(t, latest, "Latest should be nil when error occurs")
	...
	key1UUID, _ := googleUuid.NewV7()
	key1 := &cryptoutilAppsFrameworkServiceServerBarrier.IntermediateKey{
		UUID:      key1UUID,
	...
	key2UUID, _ := googleUuid.NewV7()
	key2 := &cryptoutilAppsFrameworkServiceServerBarrier.IntermediateKey{
		UUID:      key2UUID,
...
func TestGormRepository_ContentKey_Lifecycle(t *testing.T) {
	...
	intermediateKey := &cryptoutilAppsFrameworkServiceServerBarrier.IntermediateKey{
	...
	key1UUID, _ := googleUuid.NewV7()
	key1 := &cryptoutilAppsFrameworkServiceServerBarrier.ContentKey{
		UUID:      key1UUID,
	...
	key2UUID, _ := googleUuid.NewV7()
	key2 := &cryptoutilAppsFrameworkServiceServerBarrier.ContentKey{
		UUID:      key2UUID,
...
func TestGormRepository_Transaction_Rollback(t *testing.T) {
	...
	err = barrierRepo.WithTransaction(ctx, func(tx cryptoutilAppsFrameworkServiceServerBarrier.Transaction) error {
		latest, err := tx.GetRootKeyLatest()
		require.ErrorIs(t, err, cryptoutilAppsFrameworkServiceServerBarrier.ErrNoRootKeyFound, "Should return ErrNoRootKeyFound when no keys exist")
