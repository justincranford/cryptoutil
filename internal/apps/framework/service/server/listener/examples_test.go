...
var (
	...
	_testListener *cryptoutilAppsFrameworkServiceServerListener.ApplicationListener
	_baseURL      string
	_adminURL     string
)
...
func _testMainAfter(m *testing.M) {
	...
	_testListener, err = cryptoutilAppsFrameworkServiceServerListener.StartApplicationListener(ctx, cfg)
	if err != nil {
...
func _exampleHealthChecks() {
	...
	liveResult, err := cryptoutilAppsFrameworkServiceServerListener.SendLivenessCheck(_testListener.Config())
	if err != nil {
	...
	readyResult, err := cryptoutilAppsFrameworkServiceServerListener.SendReadinessCheck(_testListener.Config())
	if err != nil {
	...
	err = cryptoutilAppsFrameworkServiceServerListener.SendShutdownRequest(_testListener.Config())
	if err != nil {
