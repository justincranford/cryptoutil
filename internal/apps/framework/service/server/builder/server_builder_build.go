...
func (b *ServerBuilder) Build() (*ServiceResources, error) {
	...
	var services *cryptoutilAppsFrameworkServiceServerApplication.CoreWithServices
	...
	publicServerBase, err := newPublicServerBaseFn(&cryptoutilAppsFrameworkServiceServer.PublicServerConfig{
	...
	if services != nil {
		...
		cryptoutilAppsFrameworkServiceServerApis.RegisterRegistrationRoutes(publicServerBase.App(), services.RegistrationService, cryptoutilSharedMagic.RateLimitDefaultRequestsPerMin)
		...
		cryptoutilAppsFrameworkServiceServerApis.RegisterJoinRequestManagementRoutes(adminServer.App(), services.RegistrationService, services.SessionManager)

		// Register barrier admin endpoints (key rotation, status) on ADMIN server.
		cryptoutilAppsFrameworkServiceServerBarrier.RegisterRotationRoutes(adminServer.App(), services.RotationService)
		cryptoutilAppsFrameworkServiceServerBarrier.RegisterStatusRoutes(adminServer.App(), services.StatusService)
	}
...
func (b *ServerBuilder) applyMigrations(sqlDB *sql.DB) error {
	...
	var migrationsFS fs.FS = cryptoutilAppsFrameworkServiceServerRepository.MigrationsFS
	...
	if err := cryptoutilAppsFrameworkServiceServerRepository.ApplyMigrationsFromFS(sqlDB, migrationsFS, migrationsPath, databaseType); err != nil {
