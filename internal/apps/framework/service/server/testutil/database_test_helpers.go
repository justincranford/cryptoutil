...
func NewInitializedPostgresTestDatabase(ctx context.Context, migrationsFS fs.FS) (*sql.DB, func(), error) {
	...
	gormDB, err := cryptoutilAppsFrameworkServiceServerRepository.InitPostgreSQL(ctx, databaseURL, migrationsFS)
	if err != nil {
		closeDB()
...
func NewInitializedSQLiteTestDatabase(ctx context.Context, migrationsFS fs.FS, inMemory bool) (*sql.DB, func(), error) {
	...
	// Initialize database with migrations.
	gormDB, err := cryptoutilAppsFrameworkServiceServerRepository.InitSQLite(ctx, databaseURL, migrationsFS)
	if err != nil {
		return nil, nil, fmt.Errorf("failed to initialize database: %w", err)
	} else if gormDB == nil {
		return nil, nil, fmt.Errorf("gormDB must be non-nil")
...
func InitDatabase(ctx context.Context, databaseURL string, migrationsFS fs.FS) (*gorm.DB, error) {
	...
	case len(databaseURL) >= 11 && databaseURL[:11] == "postgres://":
		db, err = cryptoutilAppsFrameworkServiceServerRepository.InitPostgreSQL(ctx, databaseURL, migrationsFS)
	case len(databaseURL) >= cryptoutilSharedMagic.DefaultSidecarHealthCheckMaxRetries && databaseURL[:cryptoutilSharedMagic.DefaultSidecarHealthCheckMaxRetries] == "file:":
		db, err = cryptoutilAppsFrameworkServiceServerRepository.InitSQLite(ctx, databaseURL, migrationsFS)
	default:
	...
	tests := []struct {
		...
		},
		{
			...
			setupFunc: func(ctx context.Context) error {
				...
				gormDB, err := cryptoutilAppsFrameworkServiceServerRepository.InitPostgreSQL(ctx, "postgres://user:pass@nonexistent:5432/dbname", migrationsFS)
				if err != nil {
		...
		},
		{
			...
			setupFunc: func(ctx context.Context) error {
				...
				gormDB, err := cryptoutilAppsFrameworkServiceServerRepository.InitSQLite(ctx, "file:/nonexistent/invalid/path.db", migrationsFS)
				if err != nil {
