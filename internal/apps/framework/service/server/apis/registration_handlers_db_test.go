...
func TestHandleListJoinRequests_WithDB(t *testing.T) {
	...
	tenantRepo := cryptoutilAppsFrameworkServiceServerRepository.NewTenantRepository(testGormDB)
	userRepo := cryptoutilAppsFrameworkServiceServerRepository.NewUserRepository(testGormDB)
	joinRequestRepo := cryptoutilAppsFrameworkServiceServerRepository.NewTenantJoinRequestRepository(testGormDB)
	registrationService := cryptoutilAppsFrameworkServiceServerBusinesslogic.NewTenantRegistrationService(testGormDB, tenantRepo, userRepo, joinRequestRepo)
	...
	req1 := &cryptoutilAppsFrameworkServiceServerDomain.TenantJoinRequest{
		ID:          googleUuid.New(),
		UserID:      &testUserID,
		ClientID:    nil,
	...
	req2 := &cryptoutilAppsFrameworkServiceServerDomain.TenantJoinRequest{
		ID:          googleUuid.New(),
		UserID:      nil,
	...
	req3 := &cryptoutilAppsFrameworkServiceServerDomain.TenantJoinRequest{
		ID:          googleUuid.New(),
		UserID:      &testUserID,
		ClientID:    nil,
		TenantID:    testTenantID,
...
func TestHandleProcessJoinRequest_SuccessMessages(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			adminUser := &cryptoutilAppsFrameworkServiceServerRepository.User{
				ID:       adminUserID,
				TenantID: tenant.ID,
