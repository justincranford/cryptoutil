...
func TestIntegration_ListJoinRequests_NoRequests(t *testing.T) {
	...
	require.NoError(t, emptyDB.AutoMigrate(&cryptoutilAppsFrameworkServiceServerDomain.TenantJoinRequest{}))

	// Create repositories with empty DB
	joinRequestRepo := cryptoutilAppsFrameworkServiceServerRepository.NewTenantJoinRequestRepository(emptyDB)
	tenantRepo := cryptoutilAppsFrameworkServiceServerRepository.NewTenantRepository(emptyDB)
	userRepo := cryptoutilAppsFrameworkServiceServerRepository.NewUserRepository(emptyDB)

	// Create service with empty DB
	svc := cryptoutilAppsFrameworkServiceServerBusinesslogic.NewTenantRegistrationService(emptyDB, tenantRepo, userRepo, joinRequestRepo)
...
func TestIntegration_ListJoinRequests_WithData(t *testing.T) {
	...
	joinReq := &cryptoutilAppsFrameworkServiceServerDomain.TenantJoinRequest{
		ID:       googleUuid.New(),
		TenantID: testTenantID,
		UserID:   &userID,
		Status:   "pending",
	...
	var dbCount int64
	require.NoError(t, testDB.Model(&cryptoutilAppsFrameworkServiceServerDomain.TenantJoinRequest{}).Count(&dbCount).Error)
