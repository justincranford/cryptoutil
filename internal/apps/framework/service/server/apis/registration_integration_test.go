...
var (
	testDB                   *gorm.DB
	testRegistrationSvc      *cryptoutilAppsFrameworkServiceServerBusinesslogic.TenantRegistrationService
	testSessionManager       *cryptoutilAppsFrameworkServiceServerBusinesslogic.SessionManagerService
...
func (m *mockSessionValidatorIntegration) ValidateBrowserSession(ctx context.Context, token string) (*cryptoutilAppsFrameworkServiceServerRepository.BrowserSession, error) {
	...
	return &cryptoutilAppsFrameworkServiceServerRepository.BrowserSession{
		Session: cryptoutilAppsFrameworkServiceServerRepository.Session{
			TenantID: m.tenantID,
			RealmID:  m.realmID,
		},
		UserID: &m.userID,
...
func (m *mockSessionValidatorIntegration) ValidateServiceSession(ctx context.Context, token string) (*cryptoutilAppsFrameworkServiceServerRepository.ServiceSession, error) {
	...
	return &cryptoutilAppsFrameworkServiceServerRepository.ServiceSession{
		Session: cryptoutilAppsFrameworkServiceServerRepository.Session{
			TenantID: m.tenantID,
			RealmID:  m.realmID,
		},
...
func TestMain(m *testing.M) {
	...
	userRepo := cryptoutilAppsFrameworkServiceServerRepository.NewUserRepository(testDB)
	joinRequestRepo := cryptoutilAppsFrameworkServiceServerRepository.NewTenantJoinRequestRepository(testDB)

	// Create service.
	testRegistrationSvc = cryptoutilAppsFrameworkServiceServerBusinesslogic.NewTenantRegistrationService(
		testDB,
		tenantRepo,
		userRepo,
		joinRequestRepo,
	...
	testConfig := cryptoutilAppsFrameworkServiceConfig.NewTestConfig(cryptoutilSharedMagic.IPv4Loopback, 0, true)
	...
	testSessionManager, err = cryptoutilAppsFrameworkServiceServerBusinesslogic.NewSessionManagerService(
		ctx,
		testDB,
		telemetryService,
		jwkGenService,
