...
func TestInitializeServicesOnCore_ErrorPaths(t *testing.T) {
	...
	err = core.DB.Model(&cryptoutilAppsFrameworkServiceServerBarrier.RootKey{}).Count(&rootKeyCount).Error
	require.NoError(t, err)

	var intermediateKeyCount int64

	err = core.DB.Model(&cryptoutilAppsFrameworkServiceServerBarrier.IntermediateKey{}).Count(&intermediateKeyCount).Error
	require.NoError(t, err)

	var browserSessionCount int64

	err = core.DB.Model(&cryptoutilAppsFrameworkServiceServerRepository.BrowserSessionJWK{}).Count(&browserSessionCount).Error
	require.NoError(t, err)

	var serviceSessionCount int64

	err = core.DB.Model(&cryptoutilAppsFrameworkServiceServerRepository.ServiceSessionJWK{}).Count(&serviceSessionCount).Error
	require.NoError(t, err)
