...
func logSettings(s *ServiceFrameworkServerSettings) {
	if s.VerboseMode {
		...
		analysis := analyzeSettings(allServiceFrameworkServerRegisteredSettings)
...
func SetEnvAndRegisterSetting(_ []*Setting, setting *Setting) *Setting {
	...
	allServiceFrameworkServerRegisteredSettings = append(allServiceFrameworkServerRegisteredSettings, setting)
...
func validateConfiguration(s *ServiceFrameworkServerSettings) error {
	var errors []string
...
func NewForCAServer(bindAddr string, bindPort uint16, devMode bool) *ServiceFrameworkServerSettings {
	// Build args for Parse()
	args := []string{
		"start", // Subcommand required
		"--bind-public-address", bindAddr,
