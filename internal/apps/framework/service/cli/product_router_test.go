...
var testProductCfg = cryptoutilAppsFrameworkCli.ProductConfig{ //nolint:gochecknoglobals // test fixture
	ProductName: "testproduct",
	UsageText:   "Usage: testproduct <service> [options]",
	VersionText: "testproduct v1.0.0",
}

func makeTestServiceEntry(name string, exitCode int) cryptoutilAppsFrameworkCli.ServiceEntry {
	return cryptoutilAppsFrameworkCli.ServiceEntry{
		Name: name,
...
func TestRouteProduct_NoArgs(t *testing.T) {
	...
	exitCode := cryptoutilAppsFrameworkCli.RouteProduct(testProductCfg, nil, nil, &stdout, &stderr, nil)
...
func TestRouteProduct_HelpFlag(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			exitCode := cryptoutilAppsFrameworkCli.RouteProduct(testProductCfg, []string{tt.arg}, nil, &stdout, &stderr, nil)
...
func TestRouteProduct_UnknownService(t *testing.T) {
	...
	exitCode := cryptoutilAppsFrameworkCli.RouteProduct(testProductCfg, []string{"unknown-svc"}, nil, &stdout, &stderr, nil)
	require.Equal(t, 1, exitCode)
...
func TestRouteProduct_RoutesToService(t *testing.T) {
	...
	services := []cryptoutilAppsFrameworkCli.ServiceEntry{
	...
	t.Run("routes_to_svc1", func(t *testing.T) {
		...
		exitCode := cryptoutilAppsFrameworkCli.RouteProduct(testProductCfg, []string{"svc1", "server"}, nil, &stdout, &stderr, services)
		require.Equal(t, 0, exitCode)
	})
	...
	t.Run("routes_to_svc2", func(t *testing.T) {
		...
		exitCode := cryptoutilAppsFrameworkCli.RouteProduct(testProductCfg, []string{"svc2"}, nil, &stdout, &stderr, services)
...
func TestRouteProduct_MultipleServices(t *testing.T) {
	...
	services := make([]cryptoutilAppsFrameworkCli.ServiceEntry, 0, len(tests))
	...
	for i, tt := range tests {
		...
		services = append(services, cryptoutilAppsFrameworkCli.ServiceEntry{
	...
	for _, tt := range tests {
		t.Run(tt.serviceName, func(t *testing.T) {
			...
			exitCode := cryptoutilAppsFrameworkCli.RouteProduct(testProductCfg, []string{tt.serviceName}, nil, &stdout, &stderr, services)
