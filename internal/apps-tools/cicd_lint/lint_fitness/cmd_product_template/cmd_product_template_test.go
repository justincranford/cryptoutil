...
import (
	...
	_ "cryptoutil/internal/apps/` + productID + `"
)
...
// TestCheckInDir_AllValid verifies all products pass when synthetic main.go files are valid.
func TestCheckInDir_AllValid(t *testing.T) {
	...
	for _, product := range cryptoutilFitnessRegistry.AllProducts() {
		cmdDir := filepath.Join(tmpDir, "cmd", product.ID)
		require.NoError(t, os.MkdirAll(cmdDir, cryptoutilSharedMagic.FilePermOwnerReadWriteExecuteGroupOtherReadExecute))
		require.NoError(t, os.WriteFile(filepath.Join(cmdDir, "main.go"), []byte(validProductMain(product.ID)), cryptoutilSharedMagic.CacheFilePermissions))
	}
...
func TestCheckInDir_MissingMainFile(t *testing.T) {
	...
	err := CheckInDir(logger, tmpDir)
	require.Error(t, err)
	require.Contains(t, err.Error(), "cmd product template violations")
}
...
import "cryptoutil/internal/apps/sm"
func main() { _ = os.Args[1:] }
`), cryptoutilSharedMagic.CacheFilePermissions))

	errs := ExportedCheckCmdMainFile(mainPath, "sm", "cryptoutil/internal/apps/sm", true)
...
`), cryptoutilSharedMagic.CacheFilePermissions))
	...
	errs := ExportedCheckCmdMainFile(mainPath, "sm", "cryptoutil/internal/apps/sm", false)
	require.Len(t, errs, 1)
...
func TestCheckCmdMainFile_Valid(t *testing.T) {
	...
	tmpDir := t.TempDir()
	mainPath := filepath.Join(tmpDir, "main.go")
	require.NoError(t, os.WriteFile(mainPath, []byte(validProductMain("sm")), cryptoutilSharedMagic.CacheFilePermissions))
