...
// createTemplateMigrationsDirStub creates an empty template migrations directory and the
// internal/apps/ directory so that tests focused on domain migration ranges do not fail on
// the absent template dir check or the absent apps dir check.
func createTemplateMigrationsDirStub(t *testing.T, rootDir string) {
	...
	// internal/apps-framework/ no longer sits inside internal/apps/, so create it explicitly.
...
func TestCheckInDir_TemplateMigrations(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Create internal/apps/ stub so findDomainMigrationDirsWithPSID does not fail
			// before the template range check can return its violations.
