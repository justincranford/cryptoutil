...
func CheckFile(filePath string, legacyPorts []uint16, findAllFn func(string, int) [][]string) []lintPortsCommon.Violation {
	...
	// Skip framework test stub files - they define sentinel port values for test infrastructure.
	if strings.Contains(filePath, "service/testing/stubs/") {
		return nil
	}
...
func printViolations(violations []lintPortsCommon.Violation) {
	...
	fmt.Println("   sm-kms: 8700-8702 (was 8070-8072, 8888-8890)")
