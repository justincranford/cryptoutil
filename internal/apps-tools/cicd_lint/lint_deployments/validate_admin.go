...
func validateAdminNotExposed(composePath string, result *AdminValidationResult) {
	...
	for svcName, svc := range compose.Services {
		for _, portMapping := range svc.Ports {
			...
			if port == adminPort {
				result.Errors = append(result.Errors,
					fmt.Sprintf("[ValidateAdmin] Service '%s' exposes admin port %d to host in compose (SECURITY VIOLATION) | See: ENG-HANDBOOK.md Section 5.3",
						svcName, adminPort))
				result.Valid = false
			}
		}
...
func validateAdminPort(config map[string]any, configPath string, result *AdminValidationResult) {
	...
	if port != adminPort {
		result.Errors = append(result.Errors,
			fmt.Sprintf("[ValidateAdmin] '%s': bind-private-port is %d, MUST be %d | See: ENG-HANDBOOK.md Section 5.3",
...
func validateAdminAddress(config map[string]any, configPath string, result *AdminValidationResult) {
	...
	if strVal != cryptoutilSharedMagic.IPv4Loopback {
		result.Errors = append(result.Errors,
			fmt.Sprintf("[ValidateAdmin] '%s': bind-private-address is %q, MUST be %q | See: ENG-HANDBOOK.md Section 5.3",
