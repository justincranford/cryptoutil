...
func TestPoolsExample(t *testing.T) {
	...
	if err != nil {
		slog.Error("failed to generate keys", cryptoutilSharedMagic.StringError, err)
...
func generateKeys(ctx context.Context, telemetryService *cryptoutilSharedTelemetry.TelemetryService) ([]any, error) {
	rsaKeyGenPool, err1 := cryptoutilSharedPool.NewValueGenPool(cryptoutilSharedPool.NewValueGenPoolConfig(ctx, telemetryService, "Test RSA cryptoutilSharedMagic.DefaultMetricsBatchSize", exampleNumWorkersRSA, examplePoolSize, exampleMaxLifetimeKeys, cryptoutilSharedMagic.MaxPoolLifetimeDuration, cryptoutilSharedCryptoKeygen.GenerateRSAKeyPairFunction(cryptoutilSharedMagic.RSAKeySize2048), false))
	...
	aesKeyGenPool, err5 := cryptoutilSharedPool.NewValueGenPool(cryptoutilSharedPool.NewValueGenPoolConfig(ctx, telemetryService, "Test AES cryptoutilSharedMagic.TLSSelfSignedCertSerialNumberBits GCM", exampleNumWorkersOther, examplePoolSize, exampleMaxLifetimeKeys, cryptoutilSharedMagic.MaxPoolLifetimeDuration, cryptoutilSharedCryptoKeygen.GenerateAESKeyFunction(cryptoutilSharedMagic.AESKeySize128), false))
	aesHsKeyGenPool, err6 := cryptoutilSharedPool.NewValueGenPool(cryptoutilSharedPool.NewValueGenPoolConfig(ctx, telemetryService, "Test AES HS 128", exampleNumWorkersOther, examplePoolSize, exampleMaxLifetimeKeys, cryptoutilSharedMagic.MaxPoolLifetimeDuration, cryptoutilSharedCryptoKeygen.GenerateAESHSKeyFunction(cryptoutilSharedMagic.MaxUnsealSharedSecrets), false))
	hmacKeyGenPool, err7 := cryptoutilSharedPool.NewValueGenPool(cryptoutilSharedPool.NewValueGenPoolConfig(ctx, telemetryService, "Test HMAC cryptoutilSharedMagic.MaxUnsealSharedSecrets", exampleNumWorkersOther, examplePoolSize, exampleMaxLifetimeKeys, cryptoutilSharedMagic.MaxPoolLifetimeDuration, cryptoutilSharedCryptoKeygen.GenerateHMACKeyFunction(cryptoutilSharedMagic.MaxUnsealSharedSecrets), false))
	...
	keys := make([]any, 0, cryptoutilSharedMagic.GitRecentActivityDays*exampleMaxLifetimeKeys) // 7 pools * K keys per pool
