...
func TestParameterSetRegistry_HashWithAllVersions(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			hash, err := HashSecretPBKDF2WithParams(secret, params)
			...
			valid, err := VerifySecret(hash, secret)
...
func TestParameterSetRegistry_CrossVersionVerification(t *testing.T) {
	...
	hashV1, err := HashSecretPBKDF2WithParams(secret, registry.GetDefaultParameterSet())
	...
	hashV2, err := HashSecretPBKDF2WithParams(secret, paramsV2)
	...
	hashV3, err := HashSecretPBKDF2WithParams(secret, paramsV3)
