...
func TestInternalMain_HappyPath(t *testing.T) {
    ...
    stdout := &bytes.Buffer{}
    exitCode := internalMain(args, nil, stdout, nil)
...
2. Analyze HTML: `go tool cover -html=./test-output/coverage_pkg.out -o ./test-output/coverage_pkg.html`
3. Identify RED (uncovered) lines in HTML
4. Write tests ONLY for identified uncovered code
5. Re-run coverage, verify gaps filled
...
```go
// ✅ CORRECT: 5s+ timeouts with context
client := &http.Client{
...
| Factor | Local | GitHub Actions |
|--------|-------|----------------|
| Network operations | 2-5s | 5-10s |
| TLS handshakes | 5s | 10-15s |
| Multiplier | 1x | 2.5-5x |

**Flaky Test Detection**: Run `go test -count=3` to detect flakiness. Timeout errors under parallel execution = insufficient timeout, NOT test bug.

**See**: `github.md` for GitHub Actions patterns
...
**Phased Targets**: Phase 4 ≥85%, Phase 5+ ≥98% efficacy
