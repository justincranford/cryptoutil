...
- **Service tier**: value = `{PS-ID}-unseal-key-N-of-5-{hex-32-bytes}` (e.g., `sm-im-unseal-key-1-of-5-a1b2c3...`)
- **Product tier**: value = `{PRODUCT}-unseal-key-N-of-5-{hex-32-bytes}` (e.g., `sm-unseal-key-1-of-5-...`)
- **Suite tier**: value = `{SUITE}-unseal-key-N-of-5-{hex-32-bytes}` (e.g., `cryptoutil-unseal-key-1-of-5-...`)
- Each shard MUST have a unique random hex value. NEVER copy hex values across shards or services.
...
<!-- @source from="docs/ARCHITECTURE.md" as="secrets-detection-strategy" -->
...
See [ARCHITECTURE.md Section 13.3 Secrets Management in Deployments](../../docs/ARCHITECTURE.md#133-secrets-management-in-deployments) for Docker secrets enforcement in compose files.
...
See [ARCHITECTURE.md Section 6.6 JOSE Architecture & Strategy](../../docs/ARCHITECTURE.md#66-jose-architecture--strategy) for JOSE/JWK/JWS/JWE patterns, elastic key rings, and algorithm selection.
