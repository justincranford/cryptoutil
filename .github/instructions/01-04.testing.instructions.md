...
- **Comprehensive vs. sampling**: Base algorithms (e.g., RSA2048, AES256, ES256) use `TestProbAlways` (100%), variants use `TestProbThird` (33.3%) or `TestProbQuarter` (25%)
...
```go
const (
    TestProbAlways  = 1.0   // 100% - Always execute (base algorithms)
    TestProbHalf    = 0.5   // 50%  - Execute half the time
    TestProbThird   = 0.333 // 33.3% - Execute one-third of the time
    TestProbQuarter = 0.25  // 25%  - Execute quarter of the time
    TestProbTenth   = 0.1   // 10%  - Execute one-tenth of the time
    TestProbNever   = 0.0   // 0%   - Never execute (effectively skip)
)
```

**Pattern for Probability-Based Tests**:

```go
func TestCryptoAlgorithms_AllVariants(t *testing.T) {
    t.Parallel()
    tests := []struct {
        name string
        alg  Algorithm
        ...
        // Base algorithms - always test
        {name: "RSA_2048", alg: RSA2048, prob: cryptoutilMagic.TestProbAlways},
        {name: "AES_256", alg: AES256, prob: cryptoutilMagic.TestProbAlways},
        {name: "ES_256", alg: ES256, prob: cryptoutilMagic.TestProbAlways},

        // Variants - sample testing
        {name: "RSA_3072", alg: RSA3072, prob: cryptoutilMagic.TestProbThird},
        {name: "RSA_4096", alg: RSA4096, prob: cryptoutilMagic.TestProbThird},
        {name: "AES_192", alg: AES192, prob: cryptoutilMagic.TestProbQuarter},
        {name: "AES_128", alg: AES128, prob: cryptoutilMagic.TestProbQuarter},
        {name: "ES_384", alg: ES384, prob: cryptoutilMagic.TestProbThird},
        {name: "ES_512", alg: ES512, prob: cryptoutilMagic.TestProbThird},
    }
    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            t.Parallel()
            // Skip test based on probability
            if rand.Float64() > tt.prob {
                t.Skip("Skipped by probability sampling")
            }
            // test logic
            result, err := CryptoOperation(tt.alg)
...
Use helper functions to centralize probability logic (see `internal/jose/crypto/elastic_key_algorithm.go`):

```go
// GetElasticKeyAlgorithmTestProbability returns execution probability per algorithm
func GetElasticKeyAlgorithmTestProbability(alg Algorithm) float64 {
    switch alg {
    case RSA2048, AES256, ES256:  // Base algorithms
        return cryptoutilMagic.TestProbAlways
    case RSA3072, RSA4096, ES384, ES512:  // Larger key sizes
        return cryptoutilMagic.TestProbThird
    case AES128, AES192:  // Smaller key sizes
...
func TestAllAlgorithms(t *testing.T) {
    for _, alg := range allAlgorithms {
        prob := GetElasticKeyAlgorithmTestProbability(alg)
...
- Periodically run with `TestProbAlways` for all tests to ensure full coverage still achievable
- Monitor CI/CD test execution time - target 30-50% reduction for crypto-heavy test suites
