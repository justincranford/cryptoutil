...
**Config/Deployments**: `go run ./cmd/cicd lint-deployments` passes (when `configs/` or `deployments/` changed); config schema validates
**Docs**: `go run ./cmd/cicd lint-docs` passes (when ARCHITECTURE.md or instruction files changed)
