...
See [ARCHITECTURE.md Section 12.6 Secrets Management in Deployments](../../docs/ARCHITECTURE.md#126-secrets-management-in-deployments) for secret value patterns and validation.
...
**`cicd lint-deployments`** validates deployment and config structure. Run from project root: `go run ./cmd/cicd-lint lint-deployments`

**Structural Mirror**: Every `deployments/` dir MUST have `configs/` counterpart. Identity mapping: deployment directory name = config directory name (1:1). Only exception: `cryptoutil-suite` → `cryptoutil`. Orphaned configs produce warnings only — no archive directory, git history preserves all deleted files.
...
See [ARCHITECTURE.md Section 13.7 Infrastructure Blocker Escalation](../../docs/ARCHITECTURE.md#137-infrastructure-blocker-escalation) for the three-encounter escalation rule.

## Operational Excellence Cross-References

See [ARCHITECTURE.md Section 14.4 Capacity Planning](../../docs/ARCHITECTURE.md#144-capacity-planning) for resource limits, scaling triggers, and horizontal scaling.

See [ARCHITECTURE.md Section 14.5 Disaster Recovery](../../docs/ARCHITECTURE.md#145-disaster-recovery) for backup strategy, RTO/RPO targets, and DR testing.
