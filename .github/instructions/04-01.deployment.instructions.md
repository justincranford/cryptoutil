...
**`cicd lint-deployments`** validates deployment and config structure. Run from project root: `go run ./cmd/cicd lint-deployments`
...
Run from project root: `go run ./cmd/cicd <command> [command2...]`
...
**Multiple commands**: `go run ./cmd/cicd lint-text lint-go format-go` runs all three sequentially.
