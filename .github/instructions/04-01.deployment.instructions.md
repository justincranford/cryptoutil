...
<!-- @source from="docs/ARCHITECTURE.md" as="docker-compose-rules" -->
...
|---------|----------|---------------|---------------|-------------|
| Unseal N/5 | `unseal-{N}of5.secret` | `{PS-ID}-unseal-key-N-of-5-{hex-32-bytes}` | `{PRODUCT}-unseal-key-N-of-5-{hex}` | `{SUITE}-unseal-key-N-of-5-{hex}` |
...
See [ARCHITECTURE.md Section 12.6 Secrets Management in Deployments](../../docs/ARCHITECTURE.md#133-secrets-management-in-deployments) for secret value patterns and validation.
...
<!-- @source from="docs/ARCHITECTURE.md" as="cicd-command-naming" -->
...
See [ARCHITECTURE.md Section 9.10 CICD Command Architecture](../../docs/ARCHITECTURE.md#910-cicd-command-architecture) for code flow, directory structure, and enforcement invariants.
...
**Structural Mirror**: Every `deployments/` dir MUST have `configs/` counterpart. Identity mapping: deployment directory name = config directory name (1:1). Only exception: `cryptoutil-suite` → `cryptoutil`. Orphaned configs produce **errors** (blocking) — no archive directory, git history preserves all deleted files.
...
See [ARCHITECTURE.md Section 13.1.8-13.1.10](../../docs/ARCHITECTURE.md#1318-config-file-content-validation) for validation implementation details.
...
See [ARCHITECTURE.md Section 12.4.11](../../docs/ARCHITECTURE.md#13111-validation-pipeline-architecture) for pipeline diagram and detailed rules.

See [ARCHITECTURE.md Section 12.6](../../docs/ARCHITECTURE.md#133-secrets-management-in-deployments) for deployment secrets management.
...
See [ARCHITECTURE.md Section 9.4.2 Docker Desktop and Testcontainers API Compatibility](../../docs/ARCHITECTURE.md#942-docker-desktop-and-testcontainers-api-compatibility) for diagnosis checklist.
...
See [ARCHITECTURE.md Section 14.7 Infrastructure Blocker Escalation](../../docs/ARCHITECTURE.md#147-infrastructure-blocker-escalation) for the three-encounter escalation rule.

## Operational Excellence Cross-References

See [ARCHITECTURE.md Section 15.4 Capacity Planning](../../docs/ARCHITECTURE.md#154-capacity-planning) for resource limits, scaling triggers, and horizontal scaling.

See [ARCHITECTURE.md Section 15.5 Disaster Recovery](../../docs/ARCHITECTURE.md#155-disaster-recovery) for backup strategy, RTO/RPO targets, and DR testing.
...
| `lint-docs` | Documentation chunk verification and propagation validation | After ARCHITECTURE.md or instruction file changes |
