...
See [ENG-HANDBOOK.md Section 12. Deployment Architecture](../../docs/ENG-HANDBOOK.md#12-deployment-architecture) for Docker Compose patterns and conventions.
...
See [ENG-HANDBOOK.md Section 12.3.3 Secrets Coordination Strategy](../../docs/ENG-HANDBOOK.md#1233-secrets-coordination-strategy) for deployment-level secrets management across SUITE/PRODUCT/SERVICE levels.
...
See [ENG-HANDBOOK.md Section 13.3 Secrets Management in Deployments](../../docs/ENG-HANDBOOK.md#133-secrets-management-in-deployments) for secret value patterns and validation.
...
See [ENG-HANDBOOK.md Section 12. Deployment Architecture](../../docs/ENG-HANDBOOK.md#12-deployment-architecture) for multi-stage Dockerfile patterns and best practices.
...
See [ENG-HANDBOOK.md Section 13.6 Template Enforcement & Drift Detection](../../docs/ENG-HANDBOOK.md#136-template-enforcement--drift-detection) for template comparison modes, placeholder format, and rule-based linter details.
...
See [ENG-HANDBOOK.md Section 12. Deployment Architecture](../../docs/ENG-HANDBOOK.md#12-deployment-architecture) for Docker Compose optimization strategies.
...
See [ENG-HANDBOOK.md Section 10.3 Integration Testing Strategy](../../docs/ENG-HANDBOOK.md#103-integration-testing-strategy) for test container patterns and database testing.
...
See [ENG-HANDBOOK.md Section 9.7 CI/CD Workflow Architecture](../../docs/ENG-HANDBOOK.md#97-cicd-workflow-architecture) for CI/CD configuration patterns.
...
See [ENG-HANDBOOK.md Section 9.7 CI/CD Workflow Architecture](../../docs/ENG-HANDBOOK.md#97-cicd-workflow-architecture) for complete workflow catalog and patterns.

## Docker Image Pre-Pull

**ONLY for workflows using Docker** (E2E, load tests). SKIP for unit tests, linting.

See [ENG-HANDBOOK.md Section 9.7 CI/CD Workflow Architecture](../../docs/ENG-HANDBOOK.md#97-cicd-workflow-architecture) for workflow optimization patterns.
...
See [ENG-HANDBOOK.md Section 12. Deployment Architecture](../../docs/ENG-HANDBOOK.md#12-deployment-architecture) for configuration management strategies.

## Docker Verification Must Be In-Scope

**MANDATORY**: Phases that modify Docker Compose files, config files consumed by containers, cert mount paths, or any runtime-behavior artifact MUST include a Docker Compose verification step **within the same phase** (`docker compose up --wait` + health endpoint check). If Docker Desktop is unavailable, the phase is **BLOCKED — not complete**. Config-only changes without Docker verification are untested hypotheses.

**Multi-File Config Changes**: Any change spanning multiple interrelated config files (e.g., `postgresql.conf` + `pg_hba.conf` + GORM DSN + Docker volume mounts) MUST include an integration verification step exercising the full config chain in a running environment — within the same phase.

See [ENG-HANDBOOK.md Section 14.8.1 Phase Post-Mortem](../../docs/ENG-HANDBOOK.md#1481-phase-post-mortem--mandatory) for Docker verification enforcement rules.
...
See [ENG-HANDBOOK.md Section 9.7 CI/CD Workflow Architecture](../../docs/ENG-HANDBOOK.md#97-cicd-workflow-architecture) for artifact management patterns.
...
See [ENG-HANDBOOK.md Section 9.7 CI/CD Workflow Architecture](../../docs/ENG-HANDBOOK.md#97-cicd-workflow-architecture) for cost optimization strategies.
...
See [ENG-HANDBOOK.md Section 10.11 DAST Strategy](../../docs/ENG-HANDBOOK.md#1011-dast-strategy) for dynamic application security testing patterns.

## .dockerignore - MANDATORY

ALWAYS exclude dev/test artifacts. Build context should be <10MB.

See [ENG-HANDBOOK.md Section 12. Deployment Architecture](../../docs/ENG-HANDBOOK.md#12-deployment-architecture) for Dockerfile and build context optimization.
...
See [ENG-HANDBOOK.md Section 9.10 CICD Command Architecture](../../docs/ENG-HANDBOOK.md#910-cicd-command-architecture) for code flow, directory structure, and enforcement invariants.
...
See [ENG-HANDBOOK.md Section 13.1.8-13.1.10](../../docs/ENG-HANDBOOK.md#1318-config-file-content-validation) for validation implementation details.
...
See [ENG-HANDBOOK.md Section 13.1.11](../../docs/ENG-HANDBOOK.md#13111-validation-pipeline-architecture) for pipeline diagram and detailed rules.

See [ENG-HANDBOOK.md Section 13.3](../../docs/ENG-HANDBOOK.md#133-secrets-management-in-deployments) for deployment secrets management.

## Cross-References

See [ENG-HANDBOOK.md Section 12. Deployment Architecture](../../docs/ENG-HANDBOOK.md#12-deployment-architecture) for CI/CD automation, multi-stage Dockerfiles, deployment patterns, and environment strategy.

See [ENG-HANDBOOK.md Section 12.3.3 Secrets Coordination Strategy](../../docs/ENG-HANDBOOK.md#1233-secrets-coordination-strategy) for complete secrets management strategy across SUITE/PRODUCT/SERVICE deployment levels.
...
See [ENG-HANDBOOK.md Section 9.4.2 Docker Desktop and Testcontainers API Compatibility](../../docs/ENG-HANDBOOK.md#942-docker-desktop-and-testcontainers-api-compatibility) for diagnosis checklist.
...
See [ENG-HANDBOOK.md Section 14.7 Infrastructure Blocker Escalation](../../docs/ENG-HANDBOOK.md#147-infrastructure-blocker-escalation) for the three-encounter escalation rule.

## Operational Excellence Cross-References

See [ENG-HANDBOOK.md Section 15.4 Capacity Planning](../../docs/ENG-HANDBOOK.md#154-capacity-planning) for resource limits, scaling triggers, and horizontal scaling.

See [ENG-HANDBOOK.md Section 15.5 Disaster Recovery](../../docs/ENG-HANDBOOK.md#155-disaster-recovery) for backup strategy, RTO/RPO targets, and DR testing.
...
**Multiple commands**: `go run ./cmd/cicd-lint lint-text lint-go format-go` runs all three sequentially.
