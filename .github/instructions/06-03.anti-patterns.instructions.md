...
**Code Archaeology Checklist**: Read enforce_any.go, filter.go, magic_cicd.go, format_go_test.go, self_modification_test.go
...
**NEVER DO**: Write tests without checking baseline coverage first
...
**Problem**: `TestFunc_Variant1`, `TestFunc_Variant2`, `TestFunc_Variant3` → 1371-line test file (2.7× hard limit)

**ALWAYS DO**: Use table-driven tests with variants as rows
```go
tests := []struct{name string; variant int}{{"RSA_2048", 2048}, {"RSA_3072", 3072}}
...
**ALWAYS DO**: Use 10+ second timeouts for network operations in race mode
