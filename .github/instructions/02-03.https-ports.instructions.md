---
description: "HTTPS ports and TLS configuration"
applyTo: "**"
---
# Https Ports
...
**Public Prod**: <https://127.0.0.1:8080> | **Private Prod**: <https://127.0.0.1:9090>
**Public Tests**: port 0 (MANDATORY - dynamic to avoid conflicts) | **Private Tests**: port 0 (MANDATORY - avoids Windows TIME_WAIT delays)
**Containers**: 0.0.0.0 (public only), 127.0.0.1 (private - never exposed)

## ServerSettings Pattern

 ServerSettings is static settings read and merged from YAML, CMD parameters, and ENV variables.

```go
type ServerSettings struct {
    BindPublicProtocol    string   // "https" (default)
    BindPublicAddress     string   // "127.0.0.1" (default), "0.0.0.0" (containers)
    BindPublicPort        uint16   // 8080 (default for prod containers), 0 (for tests), other (for prod non-containers)
    BindPrivateProtocol   string   // "https" (default)
    BindPrivateAddress    string   // "127.0.0.1" (default for prod containers and tests), other (for prod non-containers)
...
**CRITICAL: Tests MUST use port 0 (dynamic allocation)**: Hardcoded ports cause Windows TIME_WAIT delays (2-4 minutes), breaking sequential test execution. Port 0 enables immediate reuse.
...
- Test ports: 0 (MANDATORY - dynamic allocation avoids TIME_WAIT)
- Production container ports: 9090 (recommended, never exposed to host)
- Production non-container ports: configurable (but 127.0.0.1 ALWAYS)
- Bind address: 127.0.0.1 (ALWAYS - admin localhost-only in all environments)
- TLS: HTTPS (always)

**Private HTTP APIs**:

- `/admin/api/v1/livez` - Liveness health probe (lightweight check)
- `/admin/api/v1/readyz` - Readiness health probe (heavyweight check)
...
**See**: `service-template.md` for complete ServerSettings pattern
...
- `/admin/api/v1/livez` (Liveness): Lightweight check (is process alive?), HTTP 200 OK, failure action: restart container
- `/admin/api/v1/readyz` (Readiness): Heavyweight check (is service ready?), checks DB connection, dependent services, critical resources, HTTP 200 OK, failure action: remove from load balancer (do NOT restart)
- `/admin/api/v1/shutdown` (Graceful Shutdown): Trigger shutdown sequence (stop accepting requests, drain active requests up to 30s, close connections, release resources, exit), HTTP 200 OK
