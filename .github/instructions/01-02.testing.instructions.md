...
- **Pattern**: `go test ./pkg -coverprofile=test-output/coverage_pkg.out`
...
- **CRITICAL: Be cautious with long-running test suites** (database integration tests, ORM tests)
- **Monitor test duration**: Some packages (sqlrepository, orm) can take 5-10+ minutes
- **Use targeted test runs**: Run specific test functions or packages when iterating
...
- **Design for isolation**: Each test should create unique test data (use UUIDv7 for uniqueness)
