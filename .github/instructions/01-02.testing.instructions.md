...
**Critical**: Full mutation testing can take 30-45+ minutes. Optimize for CI/CD:

1. **Package-level parallelization**: Run gremlins on packages concurrently using workflow matrix
2. **Scope reduction**: Run full suite on PRs
3. **Configuration tuning**: Adjust `workers`, `test-cpu`, `timeout-coefficient` in `.gremlins.yaml`
4. **Incremental testing**: Only test changed packages (git diff detection)
...
- **PR validation**: Critical packages only (crypto, barrier, oauth)
- **Main push**: All packages with 45min timeout
- **Nightly**: Full suite with extended timeout
...
## Flaky Test Prevention - CRITICAL

**Pattern 1: Shared Session/Resource Invalidation**

```go
// ❌ WRONG - Session created once, shared across parallel tests
func TestLogout(t *testing.T) {
    session := createSession()  // Created ONCE
    tests := []struct{ name string }{{"valid"}, {"invalid"}}
    for _, tc := range tests {
        t.Run(tc.name, func(t *testing.T) {
            t.Parallel()  // Both tests use SAME session
            // First test logs out, second test fails!
        })
    }
}
...
func TestLogout(t *testing.T) {
    tests := []struct{ name string; createSession bool }{
    ...
    for _, tc := range tests {
        t.Run(tc.name, func(t *testing.T) {
            t.Parallel()
            if tc.createSession {
                session := createSession()  // Fresh per test
            }
        })
    }
}
...
**Pattern 2: Global State Manipulation**

```go
// ❌ WRONG - Tests manipulate os.Stdout concurrently
func TestAuditLog(t *testing.T) {
    t.Parallel()  // ❌ BANNED when modifying globals
    oldStdout := os.Stdout
    defer func() { os.Stdout = oldStdout }()
}

// ✅ CORRECT - Remove t.Parallel() for global state tests
func TestAuditLog(t *testing.T) {
    // NO t.Parallel() - manipulates global os.Stdout
    oldStdout := os.Stdout
    defer func() { os.Stdout = oldStdout }()
}
```

**Pattern 3: Shared Map/Slice Writes**

```go
// ❌ WRONG - Concurrent map writes without mutex
...
- Local: `go test -race -count=2 ./...` (requires CGO_ENABLED=1)
- CI: Runs automatically in ci-race workflow
- Flaky test indicators: HTTP 401 vs 200, nil pointer panics, map concurrent write panics
