...
**Current Version**: v2.6.2 (migrated November 2025)

### Key v2 Changes

**Removed Settings** (no longer supported in v2 API):
...
- `depguard` file-scoped rules → Use custom cicd checks (see Domain Isolation below)

**Renamed Linters**:
- `wsl` → `wsl_v5` (configuration key, linter name unchanged)

**Built-in Formatters** (no configuration needed):
- **gofumpt**: Automatically applied with `golangci-lint run --fix`
- **goimports**: Automatically applied with `golangci-lint run --fix`
- Both formatters are built into golangci-lint v2, no separate installation required
- Configuration files (`.gofumpt.toml`, import settings) still respected

### Depguard Domain Isolation

**v2 Limitation**: File-scoped deny rules NOT supported

**Solution**: Custom cicd check for identity module isolation

```bash
# Run domain isolation check
go run ./cmd/cicd go-check-identity-imports

# Automatically runs in pre-commit hook (cicd-checks-internal)
```

**Blocked Imports** (identity module cannot import):
...
- `cryptoutil/internal/common/pool` - KMS infrastructure
- `cryptoutil/internal/common/container` - KMS infrastructure
- `cryptoutil/internal/common/telemetry` - KMS infrastructure
- `cryptoutil/internal/common/util` - KMS infrastructure

**Rationale**: Maintains architectural boundaries that v2's global depguard rules cannot enforce

## Linter Compliance

### CRITICAL: Universal Quality Standards

**ALL code must meet the same quality standards** - no exceptions for test code, configs, docs, or any other artifacts:

- **ALWAYS fix ALL lint and format errors** identified by pre-commit, pre-push, golangci-lint, and VS Code
- **Fix errors EVERYWHERE**: main code, production code, test code, configs, docs, scripts, examples, tools
- **NO EXCEPTIONS**: Test files, configuration files, documentation, and tooling require the same quality as production code
- **RATIONALE**: Quality issues in any code (including tests) create technical debt, reduce maintainability, and set bad examples
...
- **FIX ALL ISSUES BEFORE COMMITTING**: Run pre-commit hooks, fix all errors, iterate until clean
