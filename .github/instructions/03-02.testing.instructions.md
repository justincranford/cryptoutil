...
## CRITICAL: Coverage Targets - NO EXCEPTIONS ALLOWED

**MANDATORY: 95%+ coverage for production, 98%+ for infrastructure/utility**

- Target coverage: 95%+ production, 98%+ infrastructure (cicd), 98% utility code
- **NO EXCEPTIONS**: Every package MUST meet target, no rationalizations
- Coverage < 95% is a BLOCKING issue requiring immediate remediation
...
- Utility packages (internal/shared/*): ≥98%
...
## CRITICAL: Timeout Configuration for Network Operations - MANDATORY

**ALWAYS use generous timeouts for network operations to prevent flaky tests:**

### HTTP Client Timeouts

```go
// DON'T: No timeout (infinite wait) or short timeout
...
resp, err := client.Do(req)  // ❌ No timeout protection

// DO: Always use context with timeout
ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
defer cancel()
req, _ := http.NewRequestWithContext(ctx, "GET", url, nil)
resp, err := client.Do(req)  // ✅ Protected by context timeout
```
...
- **Shared resources**: CI/CD runners have variable CPU steal time
- **Network latency**: Even loopback connections have overhead under load
- **Parallel execution**: Concurrent tests create resource contention
- **TLS handshakes**: Crypto operations are CPU-intensive
- **1s is too short**: Use 5s+ for network operations, 10s+ for TLS

### Flaky Test Detection

```bash
# DON'T: Run once and assume stable
go test ./internal/server/application/  # ❌ May miss intermittent failures

# DO: Run 3-5 times to detect flakiness
go test -count=3 ./internal/server/application/  # ✅ Reveals intermittent issues
go test -count=5 ./internal/shared/crypto/certificate/
```

**Pattern Recognition**: Timeout errors (context deadline exceeded, i/o timeout, EOF on TLS) under parallel test execution = insufficient timeout, not test bug.
...
**Evidence** (from archived WORKFLOW-sqlrepository-TEST-TIMES.md analysis):

- Local execution: <2s
- GitHub execution: 303s (same tests, same code)
- Individual test timing: 0.00s each (infrastructure overhead, NOT test code)
- Optimization with t.Parallel(): 601s → 303s (50% reduction, but still 150× slower than local)
...
**Correct pattern** (t.Parallel() behavior in test output):
...
2. **Don't over-optimize test code**: Infrastructure overhead dominates total execution time
...
6. **Probabilistic execution**: Use TestProbTenth/TestProbQuarter for algorithm variants to reduce execution time

## CRITICAL: Table-Driven Test Patterns - MANDATORY

**ALWAYS use table-driven tests** for both happy path AND sad path test cases:

**Happy Paths Pattern** (multiple valid inputs):
...
**Sad Paths Pattern** (multiple error conditions):
...
## CRITICAL: Probability-Based Test Execution - MANDATORY

**ALWAYS use probability-based execution for table-driven tests with algorithm/key size variants:**

**Why Probability Matters**:

- **Faster test execution**: Running all key size variants (128/192/256/384/512-bit) adds minimal coverage value but significant time
- **Comprehensive vs. sampling**: Base algorithms (e.g., RSA2048, AES256, ES256) use `TestProbAlways` (100%), variants use `TestProbTenth` (10%) or `TestProbQuarter` (25%)
- **Maintains quality**: Statistical sampling ensures bugs are eventually caught without running all variants every time
...
- Mutation testing: ≥85% Phase 4, ≥98% Phase 5+ gremlins score per package (mandatory)
...
**Test Execution Time Targets** (Unit Tests Only):

- MANDATORY <15 seconds per unit test package (excludes integration/e2e tests)
...
- **Real Dependencies Preferred**: Test containers (PostgreSQL, Otel Collector Contrib), real crypto, real HTTP servers
- **Mocks ONLY for**: Hard-to-reach corner cases, external services that can't run locally (email, SMS, cloud-only APIs)
...
- **Phased Targets**: ≥85% Phase 4, ≥98% Phase 5+ (incremental quality improvement)
- Run: `gremlins unleash --tags=!integration` (excludes integration tests)
- Focus on business logic, parsers, validators, crypto operations (exclude generated code)
...
- **Optimization**: Exclude tests, generated code, vendor directories for <20min total execution
...
- **Investigation Required**: Analyze root cause, test v0.7.0+ for fixes
