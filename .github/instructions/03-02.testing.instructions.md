...
**Problem**: SQLite stores all DATETIME values in UTC, but Go's `time.Now()` respects local timezone (`$TZ`).

**Symptom**: Timestamp comparison fails in tests when system timezone is not UTC.

```
Expected: 2025-01-24T10:00:00Z (stored in SQLite as UTC)
Actual:   2025-01-24T02:00:00-08:00 (time.Now() in PST timezone)
```

**WRONG** (fails in non-UTC timezones):
```go
// ❌ time.Now() returns PST/EST/etc if $TZ is set
createdAt := session.CreatedAt
if createdAt.After(time.Now()) {
    t.Errorf("CreatedAt should be in the past")
}
```

**CORRECT** (always works):
```go
// ✅ ALWAYS call .UTC() before comparing with SQLite timestamps
createdAt := session.CreatedAt
...
```go
// ✅ Validate timestamp is within reasonable range (handles clock skew)
now := time.Now().UTC()
fiveMinutesAgo := now.Add(-5 * time.Minute)
fiveMinutesLater := now.Add(5 * time.Minute)

if session.CreatedAt.Before(fiveMinutesAgo) || session.CreatedAt.After(fiveMinutesLater) {
    t.Errorf("CreatedAt %v outside expected range [%v, %v]",
        session.CreatedAt, fiveMinutesAgo, fiveMinutesLater)
}
```

**Root Cause**: SQLite DATETIME columns normalize to UTC on insert, but Go's `time.Now()` returns local time from system clock.

**Detection**:
```bash
# Tests pass in CI (UTC timezone) but fail locally (PST/EST)
export TZ=America/Los_Angeles
go test ./... -v

# Override timezone to reproduce CI behavior
export TZ=UTC
go test ./... -v
```

**Prevention**: ALWAYS use `.UTC()` when comparing timestamps with SQLite data.
...
**Rationale**: Starting databases, servers, repositories in each test = slow execution. TestMain starts once, reuses across all tests.
...
## Testable Entry Point Pattern - MANDATORY

**ALL entry point functions (main, Cipher, IM, etc.) MUST delegate to co-located testable internal function**

**Pattern**: Thin entry point function → testable internal function with injected dependencies (args, stdin, stdout, stderr)
...
// ✅ CORRECT - Thin Cipher() delegates to testable internalCipher()
...
// internalCipher is testable - accepts injected dependencies
func internalCipher(args []string, stdin io.Reader, stdout, stderr io.Writer) int {
    // ... business logic using injected streams
    return cipher.Cipher(args)  // Delegate to actual implementation
}
```
...
func TestInternalCipher_HelpFlag(t *testing.T) {
    args := []string{"--help"}
    stdout := &bytes.Buffer{}
    exitCode := internalCipher(args, nil, stdout, nil)
    require.Equal(t, 0, exitCode)
}
```
...
7. Mutation testing: ≥85% production code, ≥98% infrastructure/utility gremlins score per package
...
**Category-Based Targets**: ≥85% production code, ≥98% infrastructure/utility efficacy
