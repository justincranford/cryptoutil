...
## Handler Testing with app.Test() - MANDATORY

**NEVER start HTTPS listeners to test individual handlers**

**Anti-Pattern**: Starting HTTPS servers repeatedly in tests triggers Windows Firewall warnings and violates TestMain pattern.

**Solution**: Use Fiber's `app.Test()` method for in-memory handler testing WITHOUT starting network listeners.
...
**Rationale**:
- NO HTTPS listener = NO Windows Firewall warnings
- Execution time: <1ms vs >1s for real server
- Respects TestMain pattern (no repeated start/stop)
- Established pattern: 20+ examples across codebase (apikey_test.go, rate_limit_test.go, handler/*_test.go)
...
**When to Use TestMain with Real Server**:
