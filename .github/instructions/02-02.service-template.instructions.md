...
**E2E Testing Helpers**: Use `internal/apps/template/testing/e2e` package for Docker Compose orchestration. See `compose.go` for `ComposeManager` that handles startup, health checks, and teardown.
...
**See**: `01-02.beast-mode.instructions.md` for complete autonomous execution patterns
...
**Template migrations**: 1001-1999 (shared infrastructure)
- 1001: Sessions table
- 1002: Barrier encryption keys  
- 1003: Realms table
- 1004: Tenants table
- 1005: Pending users table

**Domain migrations**: 2001+ (application-specific)
- Each service starts at 2001
- Never conflicts with template (1001-1999)
...
**All services should use `*FromSettings` factory functions:**
...
func NewUnsealKeysServiceFromSettings(settings *UnsealKeysSettings) (*UnsealKeysService, error) {
    ...
    return &UnsealKeysService{keyPaths: settings.KeyPaths}, nil
}
...
**Benefits**: Configuration-driven, testable (inject test settings), consistent, self-documenting

## TestMain Pattern - MANDATORY

**ALL integration tests MUST use TestMain for heavyweight resources:**
...
func TestMain(m *testing.M) {
    ...
    testServer, _ = NewFromConfig(context.Background(), cfg)
    go testServer.Start()
    testServer.WaitForReady(context.Background(), 10*time.Second)
...
**See**: [07-01.testmain-integration-pattern.instructions.md](.github/instructions/07-01.testmain-integration-pattern.instructions.md)
...
7. **Migration Versioning**: Template 1001-1999, Domain 2001+ (never conflicts)
8. **Factory Pattern**: Use `*FromSettings` functions for testable initialization
9. **TestMain Pattern**: Start server once, share across all tests
