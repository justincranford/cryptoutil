...
cryptoutil-postgres-1:
  image: cryptoutil:local  # Reuses built image (prevents 3× build time)
```
...
**Problem**: Docker silently ignores healthcheck options with underscore syntax.

**WRONG** (underscore - silently ignored):
```yaml
healthcheck:
  ...
  timeout: 5s
  retries: 3
  start_period: 30s  # ❌ WRONG - underscore silently ignored
```
...
healthcheck:
  ...
  timeout: 5s
  retries: 3
  start-period: 30s  # ✅ CORRECT - hyphen syntax
```

**Symptom**: Service marked unhealthy immediately on startup (no grace period).

**Root Cause**: Docker Compose accepts `start_period` without error but silently ignores it. Official Docker syntax is `start-period` with hyphen.

**Detection**:
```bash
# Check for underscore syntax in compose files
grep -r "start_period" deployments/*/compose.yml

# Verify healthcheck is respected
docker inspect <container_id> | jq '.[0].State.Health'
```

**Prevention**: ALWAYS use hyphen syntax for all healthcheck options (`start-period`, NOT `start_period`).

## .dockerignore Optimization - MANDATORY

**Problem**: Without `.dockerignore`, Docker sends 200MB+ of unnecessary files to build context (test outputs, coverage reports, Git history, IDE configs).

**Impact**:
- Slower build times (extra I/O, network transfer)
- Larger build context sent to Docker daemon
- Risk of accidentally including secrets in images
- Increased memory usage during build

**MANDATORY .dockerignore Pattern** (42+ exclusions):
...
# Dependencies (use go mod vendor if needed)
vendor/

# Kubernetes and deployment configs
deployments/
*.yaml
*.yml
!deployments/docker/
...
# Check build context size after adding .dockerignore
docker build --no-cache -t test-size .
# Look for "Sending build context to Docker daemon"

# Expected: 200MB+ → <10MB reduction
```

**Prevention**: ALWAYS create `.dockerignore` before first Docker build. Review and update when adding new directories.
