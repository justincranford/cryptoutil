...
- **ALWAYS use `curl`** for HTTP requests - allowlisted and auto-approved
- **NEVER use `Invoke-WebRequest`** - triggers PowerShell command blocking/authorization prompts
- **Pattern**: `curl -X POST http://127.0.0.1:8090/endpoint -H "Content-Type: application/x-www-form-urlencoded" -d "param=value"`
- **Why**: curl is allowlisted in settings.json, Invoke-WebRequest triggers complex PowerShell pipeline authorization
...
- `curl -X POST <url> -d "data"` - Make POST request with form data
- `curl -X POST <url> -H "Content-Type: application/json" -d '{"key":"value"}'` - Make POST request with JSON
- `curl -s <url>` - Silent mode (suppress progress)
- `curl -k <url>` - Allow insecure HTTPS (self-signed certs)
