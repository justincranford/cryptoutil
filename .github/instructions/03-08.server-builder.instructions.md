...
**Modes**:
- **GORMMode**: Standard GORM-based database (default for template services)
- **RawSQLMode**: Raw database/sql for services like KMS that use SQLRepository
- **DualMode**: Both GORM and raw SQL access (for hybrid services)
- **DisabledMode**: No database needed (external database management)

```go
// KMS uses raw SQL mode (SQLRepository, not GORM)
builder.WithDatabaseConfig(NewDisabledDatabaseConfig())

// Normal services use GORM mode (default)
builder.WithDatabaseConfig(NewGORMDatabaseConfig(dsn))
```

### JWT Authentication (Task 13.2)

**New Methods**: `WithJWTAuthConfig(config JWTAuthConfig)`

**Modes**:
- **JWTAuthDisabled**: No JWT validation (session-based auth only)
- **JWTAuthRequired**: All requests must have valid JWT
- **JWTAuthOptional**: JWT validated if present, pass-through if absent

```go
// Enable JWT auth alongside sessions
builder.WithJWTAuthConfig(NewJWTAuthConfig(
    JWTAuthOptional,
...
**New Methods**: `WithStrictServerConfig(config StrictServerConfig)`

Supports oapi-codegen strict server pattern for type-safe handlers:

```go
builder.WithStrictServerConfig(StrictServerConfig{
    Enabled:         true,
    BrowserBasePath: "/browser/api/v1",
    ServiceBasePath: "/service/api/v1",
    RegisterFunc: func(app *fiber.App, handlers interface{}) {
        cryptoutilOpenapiServer.RegisterHandlersWithOptions(app, handlers, opts)
    },
})
```
...
**New Methods**: `WithBarrierConfig(config BarrierConfig)`

**Modes**:
- **TemplateBarrier**: Use template's GORM-based barrier (default)
- **SharedBarrier**: Use shared barrier (repository-agnostic)
- **DisabledBarrier**: Service manages its own encryption (KMS pattern)

```go
// KMS manages its own barrier service
builder.WithBarrierConfig(NewDisabledBarrierConfig())
```
...
**Modes**:
- **TemplateWithDomainMode**: Template 1001-1004 + domain 2001+ (default)
- **DomainOnlyMode**: Only domain migrations (e.g., KMS)
- **DisabledMode**: No migrations (external management)

```go
// KMS uses its own migration system
builder.WithMigrationConfig(NewDisabledMigrationConfig())
```

### KMS Integration Example (Task 13.6-13.7)

KMS uses ServerBuilder for infrastructure while managing its own database:

```go
// internal/kms/server/builder_adapter.go
func ConfigureServerBuilderForKMS(ctx context.Context, builder *ServerBuilder, settings *Settings) error {
    return builder.
        WithDatabaseConfig(NewDisabledDatabaseConfig()).
        WithBarrierConfig(NewDisabledBarrierConfig()).
        WithMigrationConfig(NewDisabledMigrationConfig()).
        WithJWTAuthConfig(jwtConfig).
        WithStrictServerConfig(strictConfig)
}

// internal/kms/server/server.go
func NewKMSServer(ctx context.Context, settings *Settings) (*KMSServer, error) {
    // 1. Start KMS core (creates SQLRepository, barrier, business logic)
    kmsCore, err := StartServerApplicationCore(ctx, settings)

    // 2. Configure ServerBuilder for KMS (disabled barrier/migrations/database)
    builder := NewServerBuilder(ctx, settings.ServiceTemplateSettings)
    ConfigureServerBuilderForKMS(ctx, builder, settings)

    // 3. Register KMS routes
    builder.WithPublicRouteRegistration(func(app *fiber.App, res *ServiceResources) error {
        return registerKMSRoutes(app, kmsCore)
    })

    // 4. Build (provides TLS, middleware, health endpoints)
    resources, err := builder.Build()
