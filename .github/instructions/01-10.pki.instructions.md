...
applyTo: "**"
---
# PKI and Certificate Management Instructions

## CA/Browser Forum Baseline Requirements - CRITICAL
...
- RSA: Minimum 2048 bits (recommended: 3072/4096)
- ECDSA: NIST P-256, P-384, P-521 curves
- EdDSA: Ed25519, Ed448
- Hash algorithms: SHA-256, SHA-384, SHA-512 (NEVER MD5, SHA-1)
- Signature algorithms: RSA-PSS, ECDSA-SHA256/384/512, Ed25519

**Validity Periods**:

- **Subscriber Certificates**: Maximum 398 days (post-2020-09-01)
- **Intermediate CA Certificates**: Typically 5-10 years
- **Root CA Certificates**: Typically 20-25 years
...
- **Key Usage**: MUST be present and marked critical
  - TLS Server: Digital Signature, Key Encipherment
  - CA: Certificate Sign, CRL Sign
- **Extended Key Usage**: MUST be present for subscriber certificates
  - TLS Server: `id-kp-serverAuth` (1.3.6.1.5.5.7.3.1)
  - TLS Client: `id-kp-clientAuth` (1.3.6.1.5.5.7.3.2)
- **Basic Constraints**: MUST be present and marked critical for CA certificates
  - CA: TRUE
  - Path Length Constraint: Appropriate value
- **Subject Alternative Name (SAN)**: MUST be present for subscriber certificates
  - DNS names, IP addresses, or other identifiers
  - MUST include all domain names covered by certificate
- **Authority Key Identifier**: MUST be present
- **Subject Key Identifier**: MUST be present
- **CRL Distribution Points**: MUST be present if CRL supported
- **Authority Information Access**: MUST be present
  - OCSP responder URI
  - CA Issuers URI
...
- **Country (C)**: ISO 3166-1 alpha-2 code (2 characters)
- **Organization (O)**: Legal organization name
- **Common Name (CN)**: FQDN for TLS server certificates (deprecated in favor of SAN)
...
## CA Architecture Pattern - CRITICAL
...
4. **Online Root CA → Policy Root CA → Online Issuing CA** (Policy-Driven)
   - Online Root CA: Signs Policy Root CA
   - Policy Root CA: Enforces certificate policies
   - Online Issuing CA: Signs subscriber certificates
...
5. **Offline Root CA → Online Root CA → Policy CA → Online Issuing CA** (Enterprise)
   - Offline Root CA: Air-gapped, signs Online Root CA
   - Online Root CA: Signs Policy CA certificates
   - Policy CA: Enforces certificate policies
   - Online Issuing CA: Signs subscriber certificates
...
## Certificate Transparency (CT) - MANDATORY for Public CAs
...
- Enable trust between different PKI hierarchies
- Issue cross-certificates (CA A signs CA B's public key)
- Bidirectional cross-certification (mutual trust)
- Policy mapping (map certificate policies between PKIs)

**Bridge CA Architecture**:

- Central trust anchor for multiple PKIs
- Each PKI cross-certifies with bridge CA
- Transitive trust (PKI A trusts PKI B via bridge)
- Policy constraints (enforce acceptable certificate policies)
