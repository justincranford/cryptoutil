...
**CRITICAL**: Speckit presents as sequential workflow (constitution → specify → clarify → plan → tasks → analyze → implement), but **real development is iterative**.
...
### Mini-Cycle Feedback Pattern (Every 3-5 Tasks)
...
- ✅ Section I-VIII complete with no TBD placeholders
- ✅ Terminology defined (Section VIII)
- ✅ Quality gates specified (coverage, mutation, timing)
...
### Gate 3: Clarification Complete (Optional)

**Evidence Required**:

- ✅ All ambiguities resolved via Q&A
- ✅ Topical organization (not chronological)
- ✅ Sources cited (SPECKIT-CONFLICTS-ANALYSIS, CLARIFY-QUIZME dates)
...
### Gate 6: Analysis Complete (Optional)
...
- ✅ Code coverage ≥95% production, ≥100% infrastructure
- ✅ Mutation score ≥85% Phase 4, ≥98% Phase 5+
- ✅ All tests passing with <15s per unit test package
...
**Coverage**: 100% (repository/migrations_test.go)
...
# Verify health
curl -k https://localhost:8080/admin/v1/healthz  # KMS SQLite
curl -k https://localhost:8081/admin/v1/healthz  # KMS PostgreSQL 1
curl -k https://localhost:8082/admin/v1/healthz  # KMS PostgreSQL 2
```
