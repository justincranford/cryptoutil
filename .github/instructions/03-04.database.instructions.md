...
```go
// For SQLite with raw database/sql (KMS only), limit connection pool to 1
// SQLite only supports 1 concurrent writer (even in WAL mode)
sqlDB.SetMaxOpenConns(1)  // KMS uses raw database/sql without GORM
sqlDB.SetMaxIdleConns(1)

// For SQLite with GORM (all other services), use MaxOpenConns=5
// GORM transactions require separate connection from base operations
sqlDB.SetMaxOpenConns(5)  // cryptoutilMagic.SQLiteMaxOpenConnections (see sqlite-gorm.md)
...
- **MaxOpenConns=1** for raw database/sql (KMS pattern) - prevents concurrent writes
- **MaxOpenConns=5** for GORM services - transaction wrapper needs separate connection
- **Without these settings**, parallel Go tests using `t.Parallel()` will fail with database locking errors

### Reference Implementation

- KMS (raw database/sql): `internal/kms/server/repository/sqlrepository/sql_provider.go` lines 201-213
- Template (GORM): `internal/apps/template/service/server/application/application_core.go` lines 205-215
...
- Future: May add read replicas for multi-region scale
...
**Connection Pool**: MaxOpenConns=5 for GORM transactions (see [03-05.sqlite-gorm.instructions.md](.github/instructions/03-05.sqlite-gorm.instructions.md)), MaxOpenConns=1 for raw database/sql (KMS only)
**Read-Only Transactions**: NOT supported - use standard transactions or direct queries

**Magic Constants**: `cryptoutilMagic.DBSQLiteBusyTimeout` (30s)
**Note**: `cryptoutilMagic.SQLiteMaxOpenConnections = 1` is for KMS (raw database/sql). GORM services hardcode MaxOpenConns=5.

## Database DSN Patterns

**Note**: "postgres://" is the DSN scheme per libpq standard; "PostgreSQL" is the product name

**PostgreSQL**: `postgres://user:pass@127.0.0.1:5432/dbname?sslmode=disable` (dev) or `sslmode=require` (prod)
...
**SQLite with GORM**: MaxOpenConns=5, MaxIdleConns=5, ConnMaxLifetime=0 (in-memory)
**SQLite with raw database/sql (KMS only)**: MaxOpenConns=1, MaxIdleConns=1, ConnMaxLifetime=0
